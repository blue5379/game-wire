...
## Phase 24: ハルシネーション対策強化（X 公開準備）

本格的な X 公開を見据えて、記事の信頼性を底上げする。issue-008 の詳細監査でタイトル正規化バグや人物コメント・数値の捏造が複数確認されたため、プロンプト改善・grounding 強化・事後検証の3層で対応する。

### 24.1 issue-008 ハルシネーション詳細監査
- [x] 全6記事の事実主張を Web で照合し、誤り/疑い箇所を特定（詳細結果は会話ログ参照）

### 24.2 プロンプト改善（A群）
- [x] `scripts/bedrock-client.ts` の `featureSystem` から「あなたの知識から実在する有名ゲームを紹介」条項を削除（提供データ内に限定）
- [x] 全 system prompt に「人物コメント・売上数値・レビュー件数・キャラクター名・ストーリー詳細は提供データに無ければ書かない」を明記
- [x] 「不明な場合は該当セクションごと省略」を許可するルール追記
- [x] `scripts/generate-articles.ts` 本文生成の temperature 0.5 → 0.2 に低減
- [x] 記事末尾に AI 生成 disclaimer を表示（`src/pages/issue/[issueNumber]/article/[slug].astro`）

### 24.3 タイトル正規化バグ修正（Hero Company 事案）
- [x] `generateTitle()` に `titleJa` 引数を追加し、日本語/英語タイトルを明示的に分離して渡す
- [x] `titleSystem` プロンプトに「短縮・翻訳・並べ替え・改変禁止」のルールを明記
- [x] `buildUserMessage` の【ゲーム情報】に「一字一句正確に転記、改変禁止」の注記を追加
- [x] 全カテゴリの記事生成で `game.titleJa` を `generateTitle` に渡すよう修正
- [ ] 後段検証（タイトル一致チェック）は 24.5 のバリデータで実装する

### 24.4 Tavily 必須化 + Bedrock server-side web_search 有効化
- [x] `scripts/generate-articles.ts`: `TAVILY_API_KEY` 未設定時は fail fast（`ALLOW_WITHOUT_WEB_SEARCH=true` でバイパス可）
- [ ] [pending: 別途調査] Bedrock Converse `tools` への web_search 追加（Bedrock 側で対応する API を要確認）
- [ ] [pending: 別途調査] citation の出典 URL を frontmatter `sourceUrls.references` に保存

### 24.5 post-generation バリデータ実装（C群）
- [x] `scripts/validate-article.ts` 新規作成
  - [x] タイトル整合性チェック（記事タイトルにゲームタイトルが含まれるか）
  - [x] **IGDB slug 照合**（slug=company-of-heroes, title=Hero Company の誤短縮を検出）
  - [x] プラットフォーム配列を提供データと突合（FiveM の Linux/Mac、CoH の iOS のような誤りを検出）
  - [x] 本文中の「N件」「N人」「N万」「N台」などの数値を検出し、ソース不明なら警告
  - [x] 本文中の人物発言・肩書き付き人名（〜氏、CTO〜、ディレクター〜等）を検出
- [x] `scripts/validate-article.test.ts` でユニットテスト（issue-008 の各事案を再現する 24 テスト）
- [x] `data/validation/` にレポート JSON 出力
- [x] `build-issue.ts` に統合し、しきい値（high警告 5件超）を超えた場合に警告
- [x] `VALIDATION_STRICT=true` で fail させるオプション
- [x] 既存号検証ツール `scripts/validate-existing-issue.ts` + `npm run validate-issue <path>`
- [x] 過去号（issue-001/005/007/008）を検証 → 全号で 4〜10 件の high 警告を検出。対策必要性を定量的に確認
- [ ] [将来] `weekly-build.yml` での厳格モード有効化（運用安定後）

---

## Phase 23: 参考リンクに公式日本語ページを追加（Issue #7）

記事末尾の「参考リンク」セクションには現状 IGDB のリンクのみが表示されており、読者がゲームの正式情報に到達しづらい。各ゲームの公式日本語ページへのリンクを追加する。

### 事前検証結果（採用方針確定済み）

- IGDB `websites` フィールド: 実APIで10本検証 → 日本語URLは0/10 (0%)。FF16, MH Wilds, P5R 等いずれも英語/グローバルURLのみ。**採用不可**
- Metacritic: 実検証6本でいずれもページ内に Official Site リンクなし。Publisher/Developer はテキスト表示のみ。**採用不可**
- **Bedrock Claude + Web検索ツール**: 採用。1号あたり最大6本のみ調査。月24回程度のBedrock呼び出し
- 既存記事への遡及対応はしない（次号以降の新規記事のみ対応）
- 公式日本語ページが見つからなければ何も表示しない（誤情報リスク回避）

### 実装タスク

- [x] `src/content.config.ts`: `sourceUrlsSchema` に `official: z.string().optional()` を追加
- [x] `scripts/fetch-official-jp-url.ts` を新規作成
  - [x] Tavily Web検索 + Claude で日本語公式URLを検索する関数の実装
  - [x] プロンプト: ゲーム名（英語・日本語）と発売年を入力、SNS/ストア/Wiki/ファンサイト除外、不明時は null を返す指示
  - [x] HTTP HEAD によるURL生存確認（200チェック）
  - [x] `fetch-igdb.ts:863-871` の `nonOfficialPatterns` を共通化して再利用
  - [x] エラー時は null を返してログ出力のみ（fetch-data 全体を落とさない）
- [x] `scripts/fetch-data.ts`: `selectGamesForArticles()` 直後に `enrichSelectedGamesWithOfficialUrl()` を追加
  - [x] newReleases / indies / featured / classic の各ゲームに対して順次調査
  - [x] Bedrockのレート上限を考慮し直列処理
- [x] `src/components/SourceLinks.astro`: Props 型と表示リストに `official` を追加
  - [x] 表示順: 公式サイト → Steam → IGDB → OpenCritic → YouTube
  - [x] アイコン 🌐、ラベル「公式サイト（日本語）」、`.link-item.official` のホバーカラー
- [x] 動作確認
  - [x] `scripts/fetch-official-jp-url.ts` を単体実行で5-10本のゲームで検証（FF16, MHWilds, P5R で取得成功、HollowKnight/Balatoは日本語公式なしで正しくnull）
  - [x] `DEV_MODE=true npm run build-issue:dev` でエンドツーエンド確認（Resident Evil RequiemでofficilaURL取得・issue-002.mdに出力確認）
  - [x] `npm run dev` でUIのリンク表示・遷移を確認（「🌐 公式サイト（日本語）」が参考リンク先頭に表示）

### リスク

- AWS Bedrock のWeb検索ツール対応状況: 実装着手時に最新仕様を要確認。未対応なら外部検索API（Brave Search等）への切替を検討
- Claude のハルシネーション: 古い/存在しないURLを返す可能性があるため、HTTP HEAD 検証は必須
