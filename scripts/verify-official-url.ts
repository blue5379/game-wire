...
## 判定基準
- ページ本文に、そのゲームのタイトル・開発元・発売元のいずれかが明確に登場し、
  そのゲームを扱っているページであれば "match"
- ページが別のゲーム・別の製品・別の企業のサイトであれば "mismatch"
  （例: タイトルの単語が偶然一致するだけの無関係なサイトは mismatch）
- 本文が空・情報不足・判断材料が乏しく確信が持てない場合は "uncertain"
