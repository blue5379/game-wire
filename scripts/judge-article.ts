 ...
 *
 * developer/publisher/sourceUrls を「判定対象ゲームの確認済みメタデータ」として渡す。
 * 一般名・短いタイトル（例: "MOLE"）の場合に judge の Web 検索が同名別物に流れるのを
 * 防ぎ、正しいゲームに検索が届いていない状況での unverifiable 誤判定を抑制する。
 *
...
export function buildGameMetadataSection(article: GeneratedArticle): string {
  ...
  const lines: string[] = [];
  lines.push('【判定対象ゲームの確認済みメタデータ（IGDB/Steamから取得済み）】');
