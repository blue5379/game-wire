...
describe('PromptTemplates - 本文タイトル明記ルール（Issue #194 / #362 強化）', () => {
  const rule = '記事本文（特に導入部）で、紹介するゲームの正式タイトルを最低1回、提供データのとおり正確に記載すること';
  // 導入セクション直下の注意書き（Issue #362。§6.6）。
  const introNote = '※導入文の中で、紹介するゲームの正式タイトルを';
  // 強化した本文ルールの核（シリーズ名・略称代替の明示的禁止、日本語タイトル表記の許容）。
  const seriesSubstituteBan = 'シリーズ名や略称（例: "Grand Theft Auto: San Andreas" を「GTAシリーズ」）での代替も不可';
  const jaTitleAllowance = '「タイトル（日本語）」が提供されている場合はその日本語タイトルの表記でよい';
  ...
  it.each([
    ['newReleaseSystem', () => PromptTemplates.newReleaseSystem],
    ['indieSystem', () => PromptTemplates.indieSystem],
    ['classicSystem', () => PromptTemplates.classicSystem],
  ])('%s に導入セクションの正式タイトル明記の注意書きが含まれる', (_name, getPrompt) => {
    expect(getPrompt()).toContain(introNote);
  });

  it('featureSystem には導入セクションの正式タイトル明記の注意書きを含まない', () => {
    expect(PromptTemplates.featureSystem).not.toContain(introNote);
  });
  ...
  it.each([
    ...
    ['indieSystem', () => PromptTemplates.indieSystem],
    ['classicSystem', () => PromptTemplates.classicSystem],
  ])('%s の本文ルールがシリーズ名・略称での代替を明示的に禁止し、日本語タイトル表記を許容する', (_name, getPrompt) => {
    const prompt = getPrompt();
    expect(prompt).toContain(seriesSubstituteBan);
    expect(prompt).toContain(jaTitleAllowance);
  });

  // 導入セクションの注意書きは「記事のスタイル」の「日本語タイトルを優先して使用する」より
  // 近接・具体的な位置にあるため、日本語タイトル可を省くと日本語の導入文に英語正式名を
  // 置かせる方向に働き、両ルールが衝突する（PR #370 の /code-review 指摘）
  it.each([
    ['newReleaseSystem', () => PromptTemplates.newReleaseSystem],
    ['indieSystem', () => PromptTemplates.indieSystem],
    ['classicSystem', () => PromptTemplates.classicSystem],
  ])('%s では導入セクションの注意書き自身も日本語タイトル表記を許容する', (_name, getPrompt) => {
    const prompt = getPrompt();
    const introNoteIndex = prompt.indexOf(introNote);
    expect(introNoteIndex).toBeGreaterThan(-1);
    // 注意書きは1行なので、その行の中に許容文が入っていることを行単位で確認する
    const introNoteLine = prompt.slice(introNoteIndex).split('\n')[0];
    expect(introNoteLine).toContain(jaTitleAllowance);
  });
  ...
  it.each([
    ...
    '%s では導入セクションの注意書きが「## 記事のスタイル」より前（導入セクション内）に現れる',
    (_name, getPrompt) => {
      const prompt = getPrompt();
      const introNoteIndex = prompt.indexOf(introNote);
      const styleHeadingIndex = prompt.indexOf('## 記事のスタイル');
      expect(introNoteIndex).toBeGreaterThan(-1);
      expect(styleHeadingIndex).toBeGreaterThan(-1);
      expect(introNoteIndex).toBeLessThan(styleHeadingIndex);
    }
  );
});
...
describe('buildNewReleaseSystemPrompt - 枠の趣旨は企業規模を前提にしない（Issue #336）', () => {
  // 仕様 docs/article-category-spec.md §2.1:
  //   「企業規模の判定はインディー枠の除外条件としてのみ使います。新作紹介から
  //     『大手であること』という条件は撤廃されます」
  //   「企業規模は問わない。大手でもインディーでも『注目されているなら載せる』」
  // 論点A = A-1（§11.1 確定事項 #1）で決着し PR #237 でゲートを撤廃済みなので、
  // 本文生成プロンプトが「大手企業の新作」を前提にしてはならない。
  // 発売状態で分岐する 3 ブロック（§2.5）より前の冒頭にある行なので、全状態で成立する必要がある。
  const ALL_STATUSES = [null, '発売済み', '本日発売', '発売予定'] as const;

  it.each(ALL_STATUSES)('発売状態 %s のプロンプトに「大手」が現れない', (status) => {
    expect(buildNewReleaseSystemPrompt(status)).not.toContain('大手');
  });

  it.each(ALL_STATUSES)('発売状態 %s のプロンプトが枠を「注目の新作ゲーム」と説明する', (status) => {
    expect(buildNewReleaseSystemPrompt(status)).toContain(
      '注目の新作ゲームを紹介する、読み応えのある記事を書いてください。'
    );
  });

  it('ポジティブコントロール: インディー枠のプロンプトは従来どおり「話題のインディーゲーム」を名乗る', () => {
    // 企業規模を軸から降格させたのは新作紹介だけで、インディー枠の除外条件は維持されている（§5.1）。
    // 本修正が indieSystem まで薄めていないことを固定する。
    expect(PromptTemplates.indieSystem).toContain(
      '話題のインディーゲームを紹介する、読み応えのある記事を書いてください。'
    );
  });
});
