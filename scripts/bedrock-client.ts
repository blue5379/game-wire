...
/**
 * 導入セクションに置く、正式タイトルの明記を求める注意書き（feature 以外の全カテゴリ共通）。
 *
 * Issue #194 で「記事のスタイル」側にルールを置いたが守られなかったため
 * （Issue #362。第20号の名作深掘り記事が本文で一度も "Grand Theft Auto: San Andreas" と
 * 書かず「本作」「GTAシリーズ」で通した）、遵守が必要な場所＝導入セクションの直下にも
 * 同じ要求を置く。仕様: docs/article-category-spec.md §6.6
 *
 * 日本語タイトル可を明記しているのは、この注意書きが「記事のスタイル」の
 * 「日本語タイトルを優先して使用する」ルールより近接・具体的な位置にあるため。
 * 省くと日本語の導入文に英語正式名を置かせる方向に働き、両ルールが衝突する。
 */
const INTRO_TITLE_MENTION_NOTE =
  '※導入文の中で、紹介するゲームの正式タイトルを提供データのとおり最低1回記載すること（「タイトル（日本語）」が提供されている場合はその日本語タイトルの表記でよい。「本作」「このゲーム」等の代名詞や、シリーズ名・略称での代替は不可）';

/**
 * 本文への正式タイトル明記ルール（feature 以外の全カテゴリ共通）。
 * 3カテゴリに同文を複製していたものを定数化した（文言のドリフト防止）。
 * 仕様: docs/article-category-spec.md §6.6
 */
const BODY_TITLE_MENTION_RULE =
 ...
 *
 * ## 枠の趣旨に企業規模を含めない（Issue #336）
 *
 * 冒頭は「注目の新作ゲームを紹介する」であって「大手企業の新作」ではない。
 * 論点A = A-1（§11.1 確定事項 #1）で新作紹介の企業規模ゲートは撤廃され（PR #237）、
 * 仕様 §2.1 は「企業規模は問わない。大手でもインディーでも『注目されているなら載せる』」
 * と定めている。プロンプトが「大手企業の新作」を前提にすると、小規模スタジオの
 * タイトルが選ばれた号で「大手企業が贈る」のような事実に反する枕詞が付く。
 *
 * 企業規模の推測を禁じる専用ルールは足していない。「提供データに無い情報は書かない」
 * および「開発元名・発売元名…は提供データのものを正確に転記する（推測で補完しない）」
 * が既に覆っており、規模を語らせない目的で「大手」という語をプロンプトに残すのは逆効果だから。
 *
