...
describe('runCompletenessGate: mode=fail', () => {
  it('違反があると process.exit(1) が呼ばれる', async () => {
    mockHeadOk.mockResolvedValue(true);
    ...
    await expect(
      runCompletenessGate(selected, undefined, [], 'fail')
  ...
  it('違反がなければ process.exit は呼ばれない', async () => {
    mockHeadOk.mockResolvedValue(true);
...
// ─────────────────────────────────────────────────────────────────────────────
// getGateMode: PR-6 昇格後の本番デフォルトは fail
// ─────────────────────────────────────────────────────────────────────────────

describe('getGateMode: PR-6 本番デフォルト', () => {
  it('DEV_MODE 未設定かつ COMPLETENESS_GATE 未設定 → "fail"（PR-6 昇格）', () => {
    expect(getGateMode()).toBe('fail');
  });
