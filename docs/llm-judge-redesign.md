...
- 転記の崩れは決定的に検証できる性質のものなので、必要になったら `validate-article.ts` 側に `metadata-transcription-mismatch` を新設するのが正しい形。**発売日ぶんは Issue #376 / PR #386 で実装済み。残りのジャンル・種別は Issue #387 に切り出し**（§9）
...
| Issue | 内容 | 参照 |
|---|---|---|
| **#376** | **メタデータ転記の決定的検証。** ~~`発売日` / `ジャンル` / `種別` を検証するバリデータが存在せず、judge のスコープ外にしたぶんが検出の空白になる。`validate-article.ts` に `metadata-transcription-mismatch` を新設するのが正しい形~~ → **発売日ぶんは PR #386 でマージ済み（2026-09-11 実装）。** 残りの `ジャンル` / `種別` は Issue #387 に切り出し | §3.2 |
