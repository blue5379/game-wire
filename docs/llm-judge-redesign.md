...
⚠️ **「最大5本」はプロンプト上の期待値で、コードで強制されていない。** `selectFeatureGames` は `selectedTitles` を slice せずに返し（`bedrock-client.ts:1272-1277` → `generate-articles.ts:944-964`）、コード側の定数は `FEATURE_MIN_GAMES = 3`（下限）だけ。6本以上返ってきた場合、本doc の「最大20本文 / +約200KB / +約35秒」の上限を超える。**extract ループを追加するのと同じ箇所で上限を設けるか、上限が無いことを承知の上で実装すること**。→ PR #375 は**上限を設けない判断をコードコメント**（`generate-articles.ts:1233-1237`）に記録した。

**Issue #379 で確定した判断（2026-09-13）:** 上限は設けない（現状維持）。実測（発行済み全21号）では 3本7号 / 4本8号 / 5本6号 / 6本以上0号 で、上限を設けなかったことによる実害は0件。ただし本数がどこにも記録されていないため、将来6本以上が起きても気づけない。そこで **観測を入れた**: 選定本数（LLM選定本数 / 最終本数）を `ValidationReport.featureSelection` に記録し、超過時は警告ログとレポートに出す（ただしステータスには算入しない。上限超過は事故ではなく「テーマに合うゲームが多い号」で自然に起きるため）。実測では 21号連続で §8.1 / §8.2 のコスト・レイテンシ見積り（5本を最大構成として計算）を超えていない。
...
- [x] feature の extract ループで**ゲーム本数の上限を設けるか、上限が無いことを承知して実装する**（`selectFeatureGames` は slice しない。§5.2 の ⚠️）
  - 監査メモ: 「**上限を設けない**」判断をフェーズ3ループ直前のコードコメントに記録する形で実装されている（6本以上なら §5.2 / §8.2 の見積りを超えることも明記）。**#379 で 2026-09-13 に確定**（上限は設けず、選定本数を `ValidationReport.featureSelection` に記録して観測する。§5.2 / §9）
...
| Issue | 内容 | 参照 |
|---|---|---|
| **#376** | **メタデータ転記の決定的検証。** ~~`発売日` / `ジャンル` / `種別` を検証するバリデータが存在せず、judge のスコープ外にしたぶんが検出の空白になる。`validate-article.ts` に `metadata-transcription-mismatch` を新設するのが正しい形~~ → **発売日ぶんは PR #386（2026-09-11）、`種別` ぶんは Issue #387（2026-09-12）で実装済み。** `ジャンル` は Issue #387 の実測で単純照合の誤検出率が 37/37 = 100%（真のハルシネーション0件）だったため、**検証しない**判断を採った（根拠は `docs/hallucination-prevention.md` 2-6） | §3.2 |
| **#377** | **`platform-mismatch` の片方向性。** ~~「公式リストにあるものが本文から落ちた／改変された」方向は現状どこも見ていない~~ → **排他的言及（「◯◯専用」「◯◯独占」「◯◯のみ」）に限って実装（本PR）。** 省略と誤りを区別できないため全機種網羅は要求しない方針。排他語を伴わない機種の省略は未検出のまま | §3.2 |
| **#378** | **`stopReason` の伝播。** 切り詰めをパース失敗経由でなく直接検知したい場合に必要 | §6.1 |
| **#379** | **特集記事のゲーム本数の上限。** ~~`FEATURE_MIN_GAMES = 3` はあるが上限が無く、`selectFeatureGames` の戻り値は slice されない。PR #375 は「上限を設けない」判断をコードコメントに記録した状態なので、その確定または観測の追加を #379 で決める~~ → **2026-09-13 に確定: 上限は設けない（現状維持）+ 観測を追加。** 実測（発行済み全21号）が 3本7号 / 4本8号 / 5本6号 / **6本以上0号** で実害0件だったため、`slice` による強制は入れない。代わりに選定本数を `ValidationReport.featureSelection` に記録し、6本以上なら警告ログとレポートに出す（ステータスには算入しない） | §5.2 / §8.2 |
