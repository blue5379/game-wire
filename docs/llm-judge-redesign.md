...
- **特集記事のゲーム本数の上限**（§5.2）。`FEATURE_MIN_GAMES = 3` はあるが上限が無く、`selectFeatureGames` の戻り値は slice されない。プロンプトの「3〜5本」が破られてもコードは止めない
- **`generated-articles.json` のサイズ観測**（§4.4）。CI がログにも artifact にも出さないため現行サイズが未知
