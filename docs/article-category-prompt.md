...
> 🗑️ **2026-08-14: 公開済みの vol.017 / 018 / 019 を削除した**（PR #<PR>）。理由は新作紹介の本数不足で、**017 = 0 本 / 018 = 1 本（しかも DLC の `Street Fighter 6 - Year 4 アルティメットパス`）/ 019 = 0 本**（仕様は 2 本）。**号番号 17 は再利用する**（`getNextIssueNumber` はファイル名の最大値 +1 = `build-issue.ts:85-97`）ので、次の定期実行は Vol.17 として生成され、`/archive/17` は別内容に差し替わる。
