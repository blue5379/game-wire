...
# 🧭 現在地（最終更新: 2026-08-14。#26 = 早期アクセスのセッション終了時点）
...
| ブロック中 | なし |
| `monitoring`（着手しない） | **9 件**: #239 / #240 / #251 / #285 / #317 / #318 / #323 / **#227** / **#284**（後 2 件は 2026-08-14 の棚卸しで実測後に付与。**#324** は 2026-08-13 に PR #326 で解消・クローズ） |
| 計画表の外にある着手候補 | **なし**（**#330** / **#331** は PR #333 / #334、**#336** は PR #337、**#26** は PR #340 で完了・クローズ済み。いずれも 2026-08-14） |
| Open Issue の総数 | **28 件**（棚卸し前 33 件 → 5 件クローズ + #336 起票・即日クローズ + #26 クローズ + **#339 起票**）。内訳は下記「2026-08-14 の Open Issue 棚卸し」 |

> ⚠️ **キューが空になったので、次セッションは「何を着手するか」ではなく「着手すべきものがあるか」から始まる。**
> 計画表を前進させる作業は残っていない。`monitoring` の Issue を勝手に開けないこと（各 Issue が
> 再開条件として実測を要求している。下記「`monitoring` ラベルへ落とした派生 Issue」の表を見る）。
>
...
**直近セッションからの申し送り（2026-08-14。#26 = 早期アクセスのセッション）**
...
- **2 件（#227 / #284）= 測定済み型。** 2026-08-14 の棚卸しで**実測を済ませたうえで**「実害が小さいので着手しない」と判断して付与した。**着手判断のための再測定は要らない**——必要なのは下表の再開条件が満たされたかの観測だけである
