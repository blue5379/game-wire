...
`limit` を20から50に引き上げた。実測の母集団件数（33〜34件）に十分な余裕を持たせつつ、発売済み側の2軸クエリ（Issue #241 / PR #243）が既に採用している `limit 50` と値を揃えた。`scripts/fetch-igdb.test.ts` のクエリ文字列アサーションを `limit 50` に更新した（新規テスト追加なし。テスト件数は29ファイル / 1104件で変化なし）。
