...
# 🧭 現在地（最終更新: 2026-08-14。#339 の分類セッション終了時点）
...
| **次に着手する 1 件** | **なし。** 棚卸しで「高」だった **#26 も完了した**（PR #340）。以後は `monitoring` の再開条件を満たしたものだけ |
| ブロック中 | なし |
...
| 計画表の外にある着手候補 | **なし**（**#330** / **#331** は PR #333 / #334、**#336** は PR #337、**#26** は PR #340 で完了・クローズ済み。いずれも 2026-08-14。**#339** は 2026-08-14 に「中 + `monitoring`」へ分類済みで着手候補ではない） |
| Open Issue の総数 | **28 件**（棚卸し前 33 件 → 5 件クローズ + #336 起票・即日クローズ + #26 クローズ + **#339 起票**）。内訳は下記「2026-08-14 の Open Issue 棚卸し」。**棚卸しの分類表に載っていない Open Issue は #339 だけで、それも下記「棚卸し後に起票された分の分類」で処理済み** |
...
> 計画表を前進させる作業は残っていない。`monitoring` の Issue を勝手に開けないこと（**未測定型 7 件は
> 再開条件として実測を要求し、測定済み型 3 件は測定を終えたうえで着手しないと決めてある**。
> 下記「`monitoring` ラベルへ落とした派生 Issue」の表を見る）。
>
...
**直近セッションからの申し送り（2026-08-14。#339 の分類セッション。コード変更なし）**
...
上の棚卸し表は **33 件のスナップショット**なので、それ以降に起票された Issue は表に載らない。該当は **#339 の 1 件だけ**で、本セッションで分類した。
