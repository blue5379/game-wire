...
> ⚠️ **本ファイル中の「公開 19 号」「107 記事」「80 記事」等の実測値は、この削除より前の母集団を指す。** 削除直前の main は `c5c9d11` で、記事本文は `git show c5c9d11:src/content/issues/issue-018.md` の形で参照できる。**`data/validation/` の Validation Report（017 / 018 / 019）は削除していない**ので、レポート由来の実測は今も検証可能。再測定するときは母集団が **16 号**になることを前提にする。
