...
# 🧭 現在地（最終更新: 2026-08-14。Open Issue 全件棚卸し → #336 のセッション終了時点）
...
| 計画表の外にある着手候補 | **なし**（**#330** / **#331** は PR #333 / #334、**#336** は PR #337 で完了・クローズ済み。いずれも 2026-08-14） |
| Open Issue の総数 | **28 件**（棚卸し前 33 件 → 5 件クローズ + #336 起票・即日クローズ）。内訳は下記「2026-08-14 の Open Issue 棚卸し」 |
...
**直近セッションからの申し送り（2026-08-14。Open Issue 全件棚卸し → #336 のセッション）**
...
| **高** | 2 → **1** | #26（早期アクセスの概念がコードに無い。`grep` で実装 0 件）/ ~~#284~~（→ 実測後に**中**へ格下げ） |
...
| Issue | 確認した内容 |
|---|---|
| #26 | `earlyAccess` / `early_access` / `早期アクセス` の実装ヒットは **0 件**。#309 の 3 値化は「発売済み / 本日発売 / 発売予定」だけで Early Access は別軸のまま。かつプロンプトが「発売中と記載し、『発売予定』とは絶対に書かない」と**断定を強制**している（`bedrock-client.ts`） |
