...
# 🧭 現在地（最終更新: 2026-08-13。#310 のセッション終了時点）
...
| | |
|---|---|
| **§8 実装計画表** | **14 / 15 行 完了。残り 1 行** |
| **次に着手する 1 件** | **#311**（PR-G。本数不足の検出）。**§8 計画表の最後の 1 行** |
| その次 | なし（計画表は #311 で完了。以後は `monitoring` の再開条件を満たしたものだけ） |
| ブロック中 | なし |
| `monitoring`（着手しない） | #239 / #240 / #251 / #285 / #317 / #318 / #323（**#324 は 2026-08-13 に PR #326 で解消・クローズ**） |

> ⚠️ **#311 に着手する前に §9.1 保留 1（severity=high の再定義）をユーザーに確認する。** PR-G 節に明記されている。

**直近セッションからの申し送り（2026-08-13。#310 のセッション）**

- **Issue 本文の実測表は「正しい数字」でも「測り方の定義」がずれていることがある。** #310 の表の「未来方向で最も近い記念日」は実際には **+8 日以降**での最近傍で、探索窓の直後（**+7 日**）を飛ばしていた（2026-04-11 は +7 日 発明の日 / 08-22 は +7 日 焼肉の日）。記念日データ自体は実在するので grep では気づけない。**結論（過去方向のほうが近い）は不変だったので実装は止めなかったが、「8 日以降」をオフセット +7 と解釈する判断はこの再測定から出た**
- **同ファイルに残る潜在欠陥を同時に直すのは実際に安かった。** #324（`isEventOnDate` の TZ 依存）は #310 と同じ関数群を触るため同 PR で解消した。実測で `TZ=America/Los_Angeles` ではイベント 0 件週が **5 週 → 3 週**に変わり記念日が全体に 1 日ずれる（0 件週の顔ぶれも変わる）ことを確認し、`TZ=America/Los_Angeles npm run test` を回帰の門にできた。**「現に壊れていない TZ 依存」は、その TZ を実際に踏むテストを書くまで壊れていないと言えない**
- **ミュータント検証が 2 セッション連続でテストの弱点を指した。** 今回生存した 1 種（`matchSelectedEventName` の完全一致判定を殺す）は、**部分一致だけでも同じ入力を拾うため区別できない**という構造だった（#308 の「2 つの検査が同じ入力を捕まえる」と同型）。包含関係のある候補名（実データに 5 組: `肉の日 ⊂ 焼肉の日` / `猫の日 ⊂ 世界猫の日` / `猫の日 ⊂ 招き猫の日` / `クリスマス ⊂ クリスマスツリーの日` / `クリスマス ⊂ クリスマスイブ`）を使うテストを足して初めて検出できた
- **`/code-review` の採点 80 未満でも、自分で実データを測ってから採否を決める価値がある。** 今回の採点 50 の指摘（部分一致が先頭一致）は、全 365 日の窓で測ると **2026 年では発火 0 件の潜在バグ**だった。それでも 3 行で塞げるので採用した。**「潜在的」と「無害」は違う**——データ改訂で成立する
- **記事データに足したフィールドが公開 Markdown に漏れないことは、テストで固定しないと保証できない。** `formatArticleForFrontmatter` はフィールドを明示列挙する実装なので実際には漏れないが、将来 `JSON.stringify` 的な実装に変わると漏れる。`featureEvent` / `backward` / `dayOffset` が frontmatter に現れないことをテストで固定した
- **（#308 セッションから継続）#310 の測定基準は「土曜基準の 5 週」で確定した。** #308 で発行日が JST の土曜になったので、§9.3-2 が併記していた 2 つの基準（土曜 5 週 / 金曜 3 週）のうち**土曜側が実運用の基準になる**。実測で再現済み（土: 04-11 / 05-23 / 06-13 / 08-22 / 09-12、金: 05-22 / 08-21 / 09-11）。**#310 の着手時に自分で測り直す必要はあるが、期待値は 5 週**
- **発行日を 1 日動かすと 2026 年 52 週のうち 29 週でイベント件数が変わる**（実 `getEventsInRange` で全週計測）。**#310 の測定は #308 マージ後の main でやり直すこと。** #308 より前に測った数字はすべて金曜基準
- **#310 が触る `fetch-japanese-events.ts` には TZ 依存が残っている**（→ **#324**）。`isEventOnDate` がローカル時刻のゲッター（`getDate()` 等）で UTC 0 時の Date を読むため、負オフセットの TZ で 1 日ずれる（実測: `TZ=America/Los_Angeles` で `new Date('2026-08-15').getDate()` = **14**）。CI は UTC・開発機は JST なので現に壊れてはいないが、**#310 が同ファイルに後方探索を足すので、そのとき一緒に直すのが最も安い**
- **PR-C の申し送り「`buildIndieCandidates` が `publishDate` を使っていない」は #308 と同じ層ではなかった。** 選定側（`fetch-data.ts`）は PR-C で**すでに JST 基準**になっており、残っていたのは「選定が `PUBLISH_DATE` を尊重するか」＝過去日を指定した再生成の再現性の話（`fetch-data` は別プロセスで `PUBLISH_DATE` を渡されていない）→ **#323** に分離。**「同じレイヤー」という申し送りは実際には層が違った**ので、申し送りの層判定も検証対象にすること
- **ミュータント検証で生存した 1 種が、テストの弱点を正しく指し示した。** 形式チェックを外しても実在チェックが同じ入力を捕まえて例外を投げるため、`toThrow(/PUBLISH_DATE/)` では 2 分岐を区別できなかった。**「例外が飛ぶこと」だけを検証すると、複数の検査が同じ入力を捕まえる場合に片方を外しても気づけない。** エラー理由まで検証する形に直した
- **公開済みの号の `publishDate` は遡って直していない。** #308 は新しく生成される号にのみ効く。過去 19 号のうち金曜のままの 13 号はそのまま（記事本文の再生成が必要になるため、意図的にスコープ外）
**前セッション（#309）からの申し送りのうち、まだ有効なもの**
...
> **消化済みとして落とした申し送り 2 件**（#308 のセッションで消化。記録は PR-F0「実施結果」節に移した）:
> 「`buildIndieCandidates` が `publishDate` を使っていない」（→ 層が違ったので **#323** に分離）／
> 「#308 は PR-C の `getJstDateString` を再利用できる」（→ 実際に再利用した）
>
> **#310 のセッションで消化した申し送り 3 件**（記録は PR-F「実施結果」節に移した）:
> 「測定基準は土曜基準の 5 週」（→ 実測で再現。5 週すべて過去方向 1〜5 日で埋まった）／
> 「測定は #308 マージ後の main でやり直す」（→ やり直した）／
> 「#324 を一緒に直すのが最も安い」（→ 同 PR で解消・クローズ）
...
> **2026-08-13 更新**: 6〜9 のうち **7（#309）/ 8（#307）/ 6（#310）が完了**し、残るのは **9（#311。本数不足の検出）だけ**になった。
...
| **1**（繰り上げ） | **#311**（PR-G。本数不足の検出） | **§8 計画表の最後の 1 行**。着手前に §9.1 保留1（high の再定義）をユーザー確認 |
