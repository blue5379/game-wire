...
1. **選定条件に設計判断の記録が無い**。`git log -S "'sports', 'racing', 'simulation'"` のヒットは `898224c`（Initial commit）の1件のみ。仕様策定前の初期実装の残骸
