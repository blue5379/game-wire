...
# 🧭 現在地（最終更新: 2026-08-13。#308 のセッション終了時点）
...
|---|---|
| **§8 実装計画表** | **13 / 15 行 完了** |
| **次に着手する 1 件** | **#310**（PR-F。特集のイベント 0 件週フォールバック） |
| その次 | #311（PR-G） |
| ブロック中 | なし |
| `monitoring`（着手しない） | #239 / #240 / #251 / #285 / #317 / #318 / #323 / #324 |

**直近セッションからの申し送り（2026-08-13。#308 のセッション）**

- **#310 の測定基準は「土曜基準の 5 週」で確定した。** #308 で発行日が JST の土曜になったので、§9.3-2 が併記していた 2 つの基準（土曜 5 週 / 金曜 3 週）のうち**土曜側が実運用の基準になる**。実測で再現済み（土: 04-11 / 05-23 / 06-13 / 08-22 / 09-12、金: 05-22 / 08-21 / 09-11）。**#310 の着手時に自分で測り直す必要はあるが、期待値は 5 週**
...
| **1**（繰り上げ） | **#310**（PR-F。特集のイベント0件週フォールバック） | 特集がイベント0件週に重複50%へ収束する。**測定は #308 マージ後の main でやり直す**（#308 より前の数字はすべて金曜基準）。期待値はイベント 0 件週 = **土曜基準の 5 週**。同ファイル（`fetch-japanese-events.ts`）に **#324** の TZ 依存が残っているので一緒に直すのが安い |
| **2** | **#311**（PR-G。本数不足の検出） | 着手前に §9.1 保留1（high の再定義）をユーザー確認 |
...
| **#324**（`isEventOnDate` がローカル時刻のゲッターを使い負オフセット TZ で 1 日ずれる） | ①**#310（PR-F）に着手するとき**（同ファイルに後方探索を足すので同時に直すのが最も安い） ②CI ランナーの TZ が UTC 以外になったとき、または負オフセット環境で号を生成する運用が発生したとき。⚠️ 現に壊れてはいない（CI=UTC / 開発機=JST でのみ動かしているため偶然成立）。着手時は `TZ=America/Los_Angeles npm run test` で依存箇所を洗い出せる |
