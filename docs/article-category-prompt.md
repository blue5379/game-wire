...
| PR-E | `fix/issue-307-prompt-excerpt-length` | 未着手 | **#307** | PR-C と同じ箇所を触る。どちらか先に入れてリベース。**§5.6 が「最も効果の大きい修正」と位置づけている**。修正箇所は `fetch-web-search.ts` の4箇所。ブランチ名は当初案の `fix/prompt-excerpt-length` から Issue 番号入りに変更 |
...
| 8 | 抜粋 300 字 vs バリデータ 1,500 字 → ハルシネーションが「根拠あり」判定（§5.6） | ❌ **#307** | 検証 |
...
| **2** | **#307**（PR-E。抜粋 300→1,500字） | §5.6 が「最も効果の大きい修正」と明記。バリデータが 1,500 字を見てプロンプトに 300 字しか渡らない構造的な穴を塞ぐ（ハルシネーションが「根拠あり」と誤判定される） |
| **3** | **#309**（PR-C。未発売記事の分岐・3値化・JST統一） | 実際に事故が 2 件（vol.3 / vol.9）。#307 と同じ箇所を触るのでリベースが必要 |
| 4 | **#308**（PR-F0）→ **#310**（PR-F） | 特集がイベント0件週に重複50%へ収束する。#308 が #310 の測定前提 |
| 5 | **#311**（PR-G。本数不足の検出） | 着手前に §9.1 保留1（high の再定義）をユーザー確認 |
