...
# 🧭 現在地（最終更新: 2026-08-14。本番テスト実行 → 運用レイヤ修正のセッション終了時点）

> **このブロックは毎セッションの終わりに必ず更新する。** 本ファイルは 4,400 行を超えており、
> セッション冒頭に全体を掴む入口が必要なため先頭に置いてある。
> ここが古いと、次のセッションは「Issue 一覧から目についたものを選ぶ」状態に戻る。

| | |
|---|---|
| **§8 実装計画表** | ✅ **15 / 15 行 完了**（2026-08-13。#311 / PR #328 で完了） |
| **Milestone `記事カテゴリ抜本改修`** | ✅ **5 件すべて CLOSED**（#297 / #309 / #308 / #310 / #311） |
| **次に着手する 1 件** | **#350**（チェック機構の重大度設計と運用アクションの対応づけ。#156 の下位）。**着手前にユーザーとの設計判断が必要**（どの判定項をどのアクションに対応させるか / judge の確信度を運用に接続するか） |
| ブロック中 | なし |
| `monitoring`（着手しない） | **10 件**: #239 / #240 / #251 / #285 / #317 / #318 / #323 / **#227** / **#284** / **#339**（#227 / #284 は 2026-08-14 の棚卸しで、**#339** は 2026-08-14 の分類セッションで、いずれも実測後に付与。**#324** は 2026-08-13 に PR #326 で解消・クローズ） |
| 計画表の外にある着手候補 | **#350 の 1 件だけ**（**#330** / **#331** / **#336** / **#26** / **#346** / **#349** は完了・クローズ済み。**#339** は「中 + `monitoring`」で着手候補ではない） |
| Open Issue の総数 | **28 件**（棚卸し前 33 件 → 本日までに 8 件クローズ = #206 / #211 / #213 / #330 / #331 / #336 / #346 / #349、起票 = #339 / #346 / #349 / #350）。内訳は下記「2026-08-14 の Open Issue 棚卸し」と「棚卸し後に起票された分の分類」。**分類表に載っていない Open Issue は無い** |
| 本番パイプラインの健全性 | ✅ **2026-08-14 に本番で実行して確認済み**（`workflow_dispatch` / `publish_date=2026-08-08` / run 31792016284）。**6 本・4 カテゴリ充足（newRelease 2 本）は本番初**。ただし同じ実行で運用レイヤの欠陥 4 件を露出させた（下記申し送り） |

> 🗑️ **2026-08-14: 公開済みの vol.017 / 018 / 019 を削除した**（PR #343）。理由は新作紹介の本数不足で、**017 = 0 本 / 018 = 1 本（しかも DLC の `Street Fighter 6 - Year 4 アルティメットパス`）/ 019 = 0 本**（仕様は 2 本）。**号番号 17 は再利用する**（`getNextIssueNumber` はファイル名の最大値 +1 = `build-issue.ts:85-97`）ので、次の定期実行は Vol.17 として生成され、`/archive/17` は別内容に差し替わる。
> ⚠️ **本ファイル中の「公開 19 号」「107 記事」「80 記事」等の実測値は、この削除より前の母集団を指す。** 削除直前の main は `c5c9d11` で、記事本文は `git show c5c9d11:src/content/issues/issue-018.md` の形で参照できる。**`data/validation/` の Validation Report（017 / 018 / 019）は削除していない**ので、レポート由来の実測は今も検証可能。ただし**レポートのファイルパス（`validation-report-0NN.*`）は号番号から生成されるため、同じ号番号を再生成すると上書きされる**（`validate-article.ts` がファイル名を issue number から決定）。vol.017 の report は 2026-08-14 のテスト実行（run 31792016284）で既に上書きされ、本 PR で `0a0709b` から復元された。**恒久的な参照方法は `git show 0a0709b:data/validation/validation-report-0NN.json`** である。再測定するときは母集団が **16 号**になることを前提にする。

> ⚠️ **次セッションの入口は #350 だが、いきなり実装に入れない。** #350 は「どの判定項をどの
> アクションに対応させるか」というユーザーとの設計判断が前提で、コードを書く前に決める項目が
> 4 つある（Issue 本文の「検討したいこと」）。**着手 = まずその 4 点をユーザーに確認すること。**
> 計画表（§8）を前進させる作業は残っていない。`monitoring` の Issue を勝手に開けないこと
> （**未測定型 7 件は再開条件として実測を要求し、測定済み型 3 件は測定を終えたうえで着手しないと
> 決めてある**。下記「`monitoring` ラベルへ落とした派生 Issue」の表を見る）。
>
> 🚨 **前回の申し送りには「現在地の表」と「棚卸しの表」の食い違いが残っていた。** 現在地は
> 「計画表の外にある着手候補 = なし」と書いていたのに、棚卸しの表は **#26 を「高」に残したまま**
> だった（#284 だけが中へ格下げされ、#26 は移動先が書かれていなかった）。**このセッションは
> ユーザーがその食い違いを指摘して #26 に着手した。** 棚卸しで分類を動かしたら、現在地の表の
> 「着手候補」欄も同じセッションで書き換えること。

**直近セッションからの申し送り（2026-08-14。公開号の削除 → 本番テスト実行 → 運用レイヤ修正のセッション）**

このセッションは「vol.017-019 を削除する」から始まり、「本番で 1 回動かして確認する」を経て、
**運用レイヤの欠陥 4 件を発見・3 件修正**して終わった。マージした PR は #343 / #345 / #347 / #348 / #351。

- **🚨 「本番で 1 回動かす」が、19 号分の静的な突き合わせでは出てこなかった欠陥を 4 件露出させた。** これが本セッション最大の収穫である。棚卸し（08-14 午前）は Open Issue 全 33 件の本文をコードと突き合わせたが、**運用レイヤ（ワークフローの shell・レポート選択・重大度）には一切触れていなかった**。実行して初めて出たもの: ①レポート選択が別号を読む ②起票タイトルの衝突 ③要因表記の欠落 ④重大度の誤り。**コードとドキュメントの照合では見つからない層がある**
- **🚨 号番号の再利用が、辞書順ソートに依存した実装を壊した。** `weekly-build.yml` は 4 箇所で `ls data/validation/validation-report-*.json | sort | tail -1` を使っており、番号が単調増加である限り正しく動いていた（#206 / #211 は実際に起票されている）。vol.017-019 を削除して 17 を再利用した結果、**残置した 019 のレポート（status=warning）を読み、error の号を warning と判定して起票を飛ばした**（実行ログに `status=warning のため起票不要です。`）。**「今まで動いていた」は「正しい」ではない**——前提（番号の単調増加）が変わると壊れる実装だった
- **`printf "%03d" 017` は bash の 8 進数解釈で 015 になる。** Sonnet の初回実装がこれを踏んでおり、管理者レビューで検出した。ファイル名から取った番号はゼロ詰め文字列なので、数値として使うなら `$((10#$X))` で基数を固定する。`018` は 8 進数として不正なのでエラーになる。**放置すれば、レポート不在の LOUD FAILURE に当たってワークフローが commit & push の後で落ち、記事は公開されるがデプロイされない状態になっていた**
- **「最初に結果が返ったクエリで打ち切る」は、より精度の低いクエリの方が正解を返すケースで機能しない**（→ #346）。`ほの暮しの庭` の日本語公式は実在し `query[3]` で取れるのに、`query[0]`（社名入り = 最も精度が高い想定）がレビューサイトとニュース記事の 2 件を返した時点で打ち切られていた。Claude の選別は正しく `null` を返していた。**「候補が 0 件なら次へ」ではなく「使える候補が得られなければ次へ」が正しい打ち切り条件**
- **🚨 #222 の教訓（未計測と 0 件を潰すな）を管理者自身が踏み、既存テストが検出した。** #349 でサマリ表を 2 行に分けるとき、`webSearchStats` 未定義（旧キャッシュ = 未計測）を「計測して 0 件」と表示する実装にしてしまった。**doc に書いてある教訓でも、別の文脈では同じ誤りを再生産する。** 検出したのは #222 のレビュー対応で書かれた回帰テストだった——**過去のレビュー指摘をテストに落としておく価値がここで回収された**
- **judge の high 警告は `status` に届いていない**（→ #350）。本日の号で judge は確信度 90%（`ほの暮しの庭` の対応機種に PS5）と 95%（`パルワールド` の開発期間）で `llm-judge-contradicted` を出したが、`warningsBySeverity.high` は 0 で `status=error` の要因にもならない。#298 のときに記録された「judge が確信度 95% で誤情報と断じたが運用に反映されていない」と同じ構造が**まだ残っている**
- **`status=error` の効果は Issue 自動起票だけだった**（実装確認）。ビルド継続は high 警告 5 件超で判定し `status` を見ておらず（`build-issue.ts:779`）、`VALIDATION_STRICT` はワークフロー未設定、自動再生成も high ベース。**3 値設計が運用に接続されていない**ことが #350 の出発点
- **公式URL は「日本語公式 → 見つからなければ英語公式」の順で、英語フォールバックは仕様どおり**（ユーザー確認済み。無リンクより良い）。本日の `ほの暮しの庭` が英語ページになったのは #346 のバグが原因で、フォールバック自体の問題ではない。**ただしその英語ページが本文抽出に失敗して `pageContentFailures` に計上され、号が `error` になった**——1 つのバグが 2 段先の判定に波及した
- **削除したレポートのファイルパスは号番号から生成されるので、同じ号を再生成すると上書きされる。** #343 の注記で「Validation Report は削除していないので参照可能」と書いたが、テスト実行で vol.017 のレポートが上書きされ、その記述が偽になった（#348 で `0a0709b` から復元し、注記も是正）。**「消していない」は「同じ内容が残り続ける」ではない**
- **🚨 運用ミス 2 件目: 同じ作業コピーで別セッションが並行作業し、本 doc の編集が stash に退避された。** 本 docs PR を書いている最中に、別セッションが favicon 作業のために `git stash push` を実行し、**書きかけの 現在地ブロックの編集 4 箇所が stash@{0} に移された**（reflog に自分が実行していない `checkout` と `commit 7920e7c` が並ぶ）。気づいた契機は「`git status` にあった `M public/favicon.svg` が消えていた」こと。**復旧時に `git stash apply` は `error: Your local changes ... would be overwritten` で中断していたのに、`tail -3` で error 行を隠して「成功」と誤読した**——出力を切り詰めて成否を判断してはいけない。結局 stash は使わず Edit で再適用し、固定文字列（`grep -F`）で 10 箇所の存在と重複を検証した。**教訓: ①1 つの作業コピーを 2 セッションで共有すると、片方の未コミット変更がもう片方に退避・巻き込みされる ②`**` を含む文字列を `grep` に渡すと BRE として解釈され誤判定する（`-F` を使う） ③git コマンドの成否は終了コードと全出力で判断する**
- **運用ミス 1 件: squash マージされた他ブランチのコミットを自分の PR に巻き込んだ。** PR #344（favicon）が squash マージされたため元コミットが main の祖先にならず、そこから派生していた作業ブランチが favicon 2 コミットを PR #345 に持ち込んだ（内容は main と同一で実害なし）。`git rebase --onto origin/main` で分離した。**squash マージ後は、それを土台にした未マージブランチを `git log --oneline origin/main..<branch>` で確認すること**

**前セッションからの申し送り（2026-08-14。#339 の分類セッション。コード変更なし）**

- **🚨 「分離起票して本線に戻る」の後始末が漏れると、1 サイクルで同じ食い違いが再発する。** #26 のセッションで分離起票した **#339** は、**ラベルも分類も付かないまま Open** で残り、現在地の表は「計画表の外にある着手候補 = なし」と書いていた。これは前セッションが 🚨 付きで「棚卸しで分類を動かしたら現在地の表も同じセッションで直せ」と書いた**直後の再発**である（今回もユーザーの指摘で発覚した）。**進め方の「途中: 新しい欠陥を見つけたら Issue 化し本線に戻る」には、ラベル付与と再開条件の記入までを含める**（下記「再発防止」に追記）
- **judge の「同種の警告が N 件」は方向を分けて数えないと使えない。** `対応機種` に対する `llm-judge-contradicted`（severity **high**）は dev レポート **6 号**にあるが、内訳は ①**#339 のクラス（記載した機種が未発売）= 1 件**（`validation-report-011.json` の `Forza Horizon 6`。確信度 90%） ②検索で確認できない機種の過剰記載 = 3 件（013 の Mac / 018 の Linux・Mac / 024-dev の PS5） ③機種の書き落とし = 1 件（019。「対応機種は PC」と書いたが Switch 2・PS4・Xbox 版がある = **逆方向**） ④DLC と本体の混同 = 1 件（022 の `DJMAX RESPECT V`）。**6 件を #339 の実害件数として使うと 6 倍に見積もる**
- **dev レポートだけで実害頻度を語れない。** ローカルの `data/validation/`（本番）は **017 / 018 / 019 の 3 号しか無く**、そこでは `対応機種` の矛盾指摘は **0 件**。`data/validation-dev/` には 011〜024-dev が揃っているので、無意識に dev 側だけで数えると母集団が別物になる。#317 の「古いスナップショットは検出力ゼロ」と同型の、**「dev と本番でファイルの揃い方が違う」変種**
- **Issue の「データ側の材料はある」は「既に取れている」ではなかった。** #339 は `release_dates` がプラットフォーム別エントリを持つと書いていて正しいが、`IGDB_POOL_QUERY_FIELDS` が取得しているのは `release_dates.date` / `.date_format` だけで **`platform` は含まれない**（`fetch-igdb.ts:829`、型は `:867`）。#26 の「既に呼んでいる API に欲しい値が入っていた」（追加コスト 0）とは逆で、**こちらはクエリのフィールド追加が必要**

**前セッションからの申し送り（2026-08-14。#26 = 早期アクセスのセッション）**

- **🚨 「仕様に概念が無い」欠陥は、棚卸しの分類では「高」に居続けるが誰も着手しない形になりやすい。** #26 は 2026-05-29 起票で**一度も更新されていなかった**（コメント 0 件）。前セッションの棚卸しで「高」に分類されたのに、現在地の表は「着手候補なし」と書いていた。**ユーザーが食い違いを指摘して初めて着手された。** 分類を動かしたら現在地の表も同じセッションで直すこと
- **Issue の「原因（推測）」は 3 つ書かれていて、当たりは 2 つだった。** ①早期アクセスの概念が無い = **真**（実装ヒット 0 件） ②IGDB の発売日が早期アクセス開始日と 1 日ズレる = **一次ソースで再現しない**（IGDB `2026-03-05` と Steam JP ストア「2026年3月5日」が一致。judge が矛盾根拠にした「3月6日」は Tavily の検索結果由来）③プラットフォーム別発売日 = **真だが件数は小さい**（80 記事中 9 件、確定日が後発は 2 件）→ #339 に分離。**「原因の説明」は項目ごとに独立検証する**（#227 で「対処案」も検証対象と分かったのに続く実例）
- **judge の警告は「バリデータ側の偽陽性」も含む。** ②は judge が確信度 95% で「記事の 3月5日 は誤り」と断定していたが、一次ソースでは記事が正しかった。**judge の確信度は検索結果に対する確信度であって、事実に対する確信度ではない**
- **同じ 1 つのフィールドを「本文表記用」と「ゲート用」で用途分離した。** IGDB `game_status` は実測で両方向に外れる（`Realm of Ink` は 4 だが Steam は EA でない = 偽陽性 / `ARK: Survival Ascended` は null だが Steam は EA = 偽陰性。**ライブデータでも `Hot Dogs, Horseshoes & Hand Grenades` で偽陽性が再現**）。本文で偽陽性を踏むと**正式リリース済みの作品に新たな誤情報を足す**ので Steam の値だけを見る。ゲートの偽陽性は候補が 1 件減るだけなので両方見て安全側に倒す。**「精度の低いシグナルは捨てる」ではなく「偽陽性のコストが違う用途で使い分ける」**
- **🚨 IGDB の `!=` は null のレコードも落とす。** 名作枠の母集団条件に `& game_status != 4` を足すと **279 件 → 7 件**に崩壊する（名作級は `game_status=null` が普通。GTA V も Witcher 3 も null）。正しい形は `(game_status = null | game_status != 4)` で 278 件。**`games/count` で 5 通りを実測して確認した。** 除外条件を足すときは必ず件数を測る
- **既に呼んでいる API のレスポンスに、欲しい値が既に入っていることがある。** 早期アクセスの判定は Steam `appdetails` の `genres` id 70（`l=japanese` では日本語ラベル「早期アクセス」）で取れ、**この API は全 Steam 候補に対して既に呼んでいた**ので追加コストはゼロだった。新しいデータが必要だと思ったら、まず既存レスポンスの未使用フィールドを見る
- **ミュータント検証で 1 種が生存し、「自分が足した条件が死んでいる」ことを正しく指した。** `needsCompletion` に足した「早期アクセス未判定」条件を外してもテストが落ちなかった。原因は本物で、`steamRecommendations` を書くのが同じループだけなので**1 周目は必ず未設定になり他の条件が先に true になる**。#280 の「第2引数が到達不能だった」と同型。純関数 `needsStorefrontCompletion` に切り出して単体テストで意図を固定し解消した。**「防御的に足した条件」はミュータント検証で死活が分かる**
- **DEV_MODE の実データ検証が「今週まさに壊れる」ことを示した。** 今週の選定 5 本のうち **2 本が早期アクセス**だった（`Sandustry サンダストリー` = 新作 / `Backrooms: Escape Together` = インディー）。修正前ならこの 2 本が完成品として紹介されていた。**着手前の 19 号分の実測（10 記事）だけでなく、着手後のライブ選定でも実害が出ていた**
- **プロンプト修正の効き方は 3 か所で確認できた**（見出し・📅 発売情報・要約）。EA の 2 本は全箇所で「早期アクセス配信中」と書き、`isEarlyAccess=false` の `Big Walk` と**未判定**の `Splatoon Raiders` は早期アクセスに触れなかった。**未判定で何も書かせない設計が実データで確認できた**
- **`/code-review` は 5 エージェントすべて指摘ゼロだった**（採点対象 0 件のため PR コメントは投稿されていない）。過去の指摘パターン（#299/#305 の dedup マージ漏れ・#271 の未計測/0件の区別・#333 の死んだコード）はいずれも先に潰してあった

**前セッションからの申し送り（2026-08-14。Open Issue 全件棚卸し → #336 のセッション）**

- **🚨 この doc は「着手すべきものがあるか」を管理していたが、Open Issue 自体の陳腐化は管理していなかった。** 全 33 件の本文をコードと突き合わせただけで **5 件がクローズできた**（#38 / #163 / #175 / #206 / #211）。うち **#38 は対応案 1・2 の両方が実装済み**（`game-history.ts:32` の `feature: 17` にはコメントで Issue 番号まで書かれていた）、**#175 は完了条件 4 項目すべてを PR #237 が満たしていた**。**キューが空になった後は「残っている Issue が今も有効か」を定期的に問い直す必要がある**
- **「HIGH 3 件」は 3 つの欠陥ではない。** #206（第17号）の内訳は実測で **①`platform-mismatch` の誤検出 1 件 ②同一箇所の 2 重計上 1 件**（本文の「1200万本」は 1 回しか出現せず `excerpt` も完全一致）だった。**警告件数を欠陥件数として優先順位を決めると誤る**
- **バリデータ側の欠陥 2 件を検出したが、ユーザー判断で Issue 化せず記録に留めた**（発火頻度が低く読者に見える実害が無いため）。再開条件は「HIGH 件数が運用判断（`status=error` 判定・自動起票・自動再生成）を実際に誤らせたと観測されたとき」
  1. **特集記事の数値警告が必ず 2 重に出る**: `validateArticle`（`validate-article.ts:1014` と `:1017`）が `validateNumericClaims`（全カテゴリ）と `validateFeatureNumericClaims`（feature のみ）の両方を呼び、どちらも同じ `NUMERIC_PATTERNS` で同じ本文をスキャンする
  2. **`platform-mismatch` の構造的な誤検出**: 本文の「Steam」は canonical `PC (Steam)` に正規化されるのに、提供データの `platforms: ['PC']` は `KNOWN_PLATFORM_PATTERNS`（`:127-139`）に `PC` 単体のパターンが無いため正規化されない。実測で公開 19 号の platforms 値は `PC (Microsoft Windows)` 75 件に対し `PC` 単体 7 件（記事 6 本）、3 号分のレポートで発火 1 件
- **🚨 古いスナップショットで測ると検出力ゼロになる罠を今回も踏んだ。** #284 を `data/aggregated.json` で測ろうとしたが、**`steamRawDeveloper` を 1 件も持っていなかった**（このフィールドは PR #299 / #300 で後付け。`fetchedAt` は 2026-05-16）。#317 が警告している罠と同型で、**「フィールドがいつ追加されたか」も検出力を左右する**
- **Issue の「対処案」が実測で的を外していることがある。** #227 の対処案 (a)（`getAppDetails` を `fetchWithRetry` 化）は、**実際に落ちている 2 件を 1 件も救えない**。45 件すべて HTTP 200 で 429 は 0 件、欠落はいずれも **HTTP 200 + `success:false`**。しかもうち 1 件は **`cc=jp` 固有**（`War Thunder` = 日本ストア非配信。`cc=us` なら取得可・本日の top_played rank=18）で、**除外は仕様上むしろ妥当な可能性が高い**。「原因の説明」だけでなく**「対処案」も独立検証の対象**
- **#284 の実害は交差条件でしか出ない。** Steam 生値が弾かれるのは 42 件中 9 件（21.4%）だが、**8 タイトル中 6 件は IGDB が `developer` を供給して表に出ない**。実害は「**IGDB 未収録 × 単一トークン社名**」だけで実測 1 件（`Shadow of Juarez` / `Shadowline`）。落ち方は「`isQualifiedGame` は `steamRank != null` で**無条件通過**するのに `finalizeGameMetadata` が `still-missing-required` で落とす」（`game-filter.ts:44` / `finalize-game-metadata.ts:280-282`）。**「品質は満たすのに開発元名の形式だけで落ちる」**という構造
- **「決着済みだが未実装」の次に「実装済みだがプロンプトが未追随」という抜け方がある**（→ #336）。論点A の実装（PR #237、2026-08-09）から**本文プロンプトの追随が 5 日間漏れていた**。しかも**ゲート撤廃後に生成された号はまだ 1 号も無く**（最新の公開号 vol.019 は 2026-08-07 発行）、次の定期実行が初回の露出だった。**§11.1 の確定事項と実装の突き合わせでは、選定ロジックだけでなくプロンプト文面も対象にする**
- **部分的な文言修正は、残した側と矛盾しないか確認する。** #336 で冒頭行だけを直した結果、残した理由付け「枠の趣旨と合わないラベルになる」がコードと矛盾した（`pickNewReleaseLabelCompany` は両方が静的リスト外なら `return developer` なので、小規模スタジオ名がラベルになるのは正常動作）。`/code-review` の**採点 75 を管理者判断で採用**して直した
- **`import` 1 行の追加もファイル全体の行番号をずらす。** 本セッションの行番号測定で、**PR #326 の表が記録した `generate-articles.ts` の位置が着手前に既に +1 ずれていた**ことが判明した。原因は **PR #333（#330）の `f754882` が足した `import { isMainModule } from './entrypoint.js';` の 1 行**で、PR #333 の docs はこの副作用を記録していなかった。**「行数が変わる変更」はロジック追加だけではない**——import・コメント行の増減も同じ効果を持つ（本 PR も `generate-articles.ts` はコメントを 3 → 7 行にしただけで +4 ずれた）

**前セッション（#311 / #330 / #331）からの申し送り**

- **Issue 本文の実測表は「測り方」だけでなく「測った時点」も陳腐化する。** #311 の「17 号中 3 号」は着手時の実測で **19 号中 5 号**（013 / 015 / 017 / 018 / 019）になっており、さらに**直近 5 号のうち 4 号が不足**していた。#310 の申し送り（「正しい数字でも測り方の定義がずれる」）と合わせて、**Issue 本文の数値表は 2 通りの理由で信用できない**。着手時に必ず数え直す
...
- **`/code-review` が「推論」と留保した指摘でも、実測すると本物のことがある。** #330 の symlink 指摘（採点 75）はエージェント自身が「node の ESM が symlink を解決するかは推論」と書いていたが、実際に symlink を作って測ると**再現した**（argv[1] は symlink パス / `import.meta.url` は実パス → 判定 false → **CLI 実行なのに main() が呼ばれず黙って終了**）。**留保付きの指摘を留保のまま却下しない。** 測る手段があるなら測る
- **レビューの提案どおり直すと逆方向を取りこぼすことがある**（既存の規約「レビューの提案をそのまま採用しないこと」の実例が 1 件増えた）。#330 のレビュー提案は「`realpathSync` を通す」だったが、それだけでは `--preserve-symlinks` 実行（`import.meta.url` が symlink 側になる）を落とす。**素のパスと realpath の両方を候補にする**形に変えた。ミュータント検証で両方向を固定した
- **自分で書いた Issue でも対象を数え落とす。** #331 は自分が起票したのに `content-card-issues` の 4 箇所だけを挙げ、**同じカード内の `content-card-count` 4 箇所を見落としていた**（合計 8 箇所）。着手時に CSS を読んで初めて気づき、後者は `opacity: 0.08` の装飾ウォーターマークなので対象外と判断した。**「Issue が正確でも範囲が狭すぎる」は自分が書いた Issue にも起きる**
- **`validate-existing-issue.ts` は公開済み号に対する安全な検証手段として使える**（`--verify-urls` を付けなければ外部 API を叩かない。出力先は gitignore 済みの `data/validation-manual/`）。#330 の CLI 動作確認に使ったところ、副産物として **#311 の hidden 対応が実データで効いていることを 2 件確認できた**（vol.001 で `[インディーゲーム] 1 / 2 本`、vol.013 で `[名作深掘り] 0 / 1 本`）。#311 では予測として記録していた挙動
- ⚠️ **運用ミス: `git checkout -b` が意図どおり効かず、別ブランチにコミットした。** #331 の変更を #330 のブランチに乗せてしまい、cherry-pick で分離した（`gh pr view <n> --json files` で PR の中身を確認して発覚）。**コミット直前に `git status -sb` でブランチ名を確認すること。** 特に 1 セッションで複数 PR を出すときに起きる
...
- **`/code-review` の採点 80 未満でも、自分で実データを測ってから採否を決める価値がある。** #310 では採点 50 の指摘が「2026 年では発火 0 件の潜在バグ」だったが 3 行で塞げるので採用した。**「潜在的」と「無害」は違う**——データ改訂で成立する
- **記事データに足したフィールドが公開 Markdown に漏れないことは、テストで固定しないと保証できない。** `formatArticleForFrontmatter` はフィールドを明示列挙する実装なので実際には漏れないが、将来 `JSON.stringify` 的な実装に変わると漏れる
- **`IGDB enrich rejected` のログは `fetch-data.ts:113` と `:321` で別物。** #239 の監視解除条件は `:321` のみ（`:113` は該当しない）

> **#311 のセッションで消化した申し送り 1 件**（記録は PR-G「実施結果」節に移した）:
> 「#311 に着手する前に §9.1 保留 1 をユーザーに確認する」（→ 確認し、**high 警告として数えず独立判定項にする**で決着）
...
| PR-F0 | `fix/issue-308-publish-date-jst` | ✅ **マージ済み**（2026-08-13。マージコミット `713baa5`。通常マージ、squashではない） | **#308**（Closed）/ 分離 **#323** / **#324** / PR #322 | 30ファイル / **1364テスト**（着手前 30 / 1337。新規27件）。コミット1本（`b92c99b`。`/code-review` は**指摘ゼロ**だったため追加コミットは無い）。§9.3-2 が挙げた 2 案のうち **「生成側で JST に変換する」**を採用（ワークフロー側で計算する案は schedule 経路しか覆えず、境界値テストもテストスイートの外に出るため）。`scripts/jst-date.ts` に `resolvePublishDateString` を新設し、`generate-articles.ts` の 1 行（旧 `:1309`）を差し替えた。**発行日の決定点が 1 箇所であることを確認済み**（`build-issue.ts` は `generated-issue.json` の値を読み直すだけ）。実測で金曜になっていたのは **19 号中 13 号**（起票時点の「17 号中 11 号」から増えていた）。**イベント窓は 2026 年 52 週のうち 29 週で件数が変わり、イベント 0 件週は金曜基準 3 週 / 土曜基準 5 週**（§9.3-2 の記録を再現。→ #310 の測定基準は土曜側で確定）。スコープに足した判断が 1 件（不正な `PUBLISH_DATE` を生成前に落とす。従来は Bedrock のコストを払い切った後に `build-issue.ts` の `format()` で落ちていた）。ミュータント検証 8 種すべて検出だが、**うち 1 種は初回に生存**しており、テストの弱点（2 つの検査が同じ入力を捕まえるため `toThrow` だけでは区別できない）を正しく指した。ブランチ名は当初案の `fix/publish-date-jst` から Issue 番号入りに変更 |
| PR-F | `fix/issue-310-feature-event-fallback` | ✅ **マージ済み**（2026-08-13。マージコミット `6bd8041`。通常マージ、squashではない） | **#310**（Closed）/ **#324**（Closed。同 PR で解消） / PR #326 | 31ファイル / **1432テスト**（着手前 30 / 1364。**新規1ファイル・+68テスト**）。コミット2本（実装 `ec95674` → `/code-review` 指摘対応 `20abe2e`）。`fetch-japanese-events.ts` に `selectFeatureEventCandidates` を新設し 3 段階フォールバックにした。**段階 1 の結果は従来の窓と同一**（2026 年 52 土曜のうち 47 週で一致することをテストで固定）。下位判断 2 件を決定（記録先 = `history.json` の `featureEvents` / 除外号数 **N=2**）。**Issue 本文の実測表と 1 点食い違い**（「未来方向で最も近い記念日」が実際は +8 日以降での最近傍で、+7 日を飛ばしていた。結論は不変）。**#324 の TZ 依存を同時解消**し `TZ=America/Los_Angeles npm run test` を回帰の門にした。ミュータント検証 12 種すべて検出だが、**うち 1 種は初回に生存**（完全一致と部分一致が同じ入力を拾うため区別できなかった）。`/code-review` の 2 件はいずれも採点 80 未満（50 / 75）で PR コメントは投稿されず、**採点 50 の方だけ管理者判断で採用**（実データで発火 0 件の潜在バグだが 3 行で塞げるため） |
| PR-G | `feat/issue-311-article-count-validation` | ✅ **マージ済み**（2026-08-13。マージコミット `6567ed1`。通常マージ、squashではない） | **#311**（Closed）/ PR #328 | 31ファイル / **1469テスト**（着手前 31 / 1432。新規37件）。コミット1本（`6e13dc7`。`/code-review` は**5エージェントすべて指摘ゼロ**だったため追加コミットは無い）。**§8 実装計画表の最後の 1 行 = これで 15/15 完了。** severity は仕様 §6.5 の「high」を採らず、**high 警告に混ぜない独立した判定項**（`ValidationReport.articleCountShortfalls`）にした（ユーザー確認済み。理由は §9.1 保留1 との切り離しと、`warningsBySeverity.high` が fail 閾値・自動再生成にも使われる数値だから。前例は #222 の `adultScreeningFailures`）。下位判断（§9.2-10）は**定数 + 環境変数 `EXPECTED_ARTICLE_COUNT_*` の上書き**で決定。**着手前検証で Issue 本文の実測表が陳腐化していた**（「17 号中 3 号」→ 実測 **19 号中 5 号**。直近 5 号のうち 4 号が不足）だけでなく、**予測された実害が起票の 3 日前に本番で発生済み**だった（vol.019 が `status=warning` で Issue 未起票）。hidden 記事を本数から除外する判断を追加（実測で該当は vol.001 の 1 件のみ）。実データで公開済み 19 号すべての判定を再現し、**status が変わるのは vol.019 の warning → error だけ**・正常な 14 号は**誤検出 0 件**を確認。ミュータント検証 **9 種**すべて検出（2 セッション続いた「1 種生存」が止まった）。**副産物: エントリポイントガードを持たないスクリプトが 3 本あることを発見**（未起票。下記「実施結果」節） |
...
| **#330 対応** | `fix/issue-330-entrypoint-guard` | ✅ **マージ済み**（2026-08-14。マージコミット `0817339`。通常マージ、squashではない） | **#330**（Closed）/ PR #333 | 33ファイル / **1486テスト**（着手前 31 / 1469。**新規テストファイル2本（+17テスト）と新規モジュール `scripts/entrypoint.ts`**）。コミット2本（実装 `f754882` → `/code-review` 指摘対応 `b07406b`）。**§8 計画表の外**（#311 の作業中に発見した別層の欠陥）。`main()` をガード無しでトップレベル実行していた 3 本（`build-issue.ts` / `fetch-data.ts` / `validate-existing-issue.ts`）に `isMainModule()` を追加し、既存でガードを持っていた 2 本も共有ヘルパー `scripts/entrypoint.ts` に寄せた。**判定を文字列連結から `pathToFileURL` に変え、既存 2 本が持っていた穴（空白・`#` を含むパスで CLI が no-op になる）も塞いだ。** `/code-review` の指摘 1 件（採点 75。symlink 経由で CLI が no-op になる）は**エージェント自身が「推論」と留保していたが実測で再現したため採用**し、ただし**提案どおり realpath だけを見ると `--preserve-symlinks` 実行を落とす**ため両方を候補にする形に変えた。ミュータント検証 9 種のうち 8 種検出（1 種は防御的な行で挙動を定義していないことが判明し、理由をコメントに残した） |
| **#331 対応** | `fix/issue-331-content-card-counts` | ✅ **マージ済み**（2026-08-14。マージコミット `21b6d6e`。通常マージ、squashではない） | **#331**（Closed）/ 関連 **#311**（本 Issue の出所） / PR #334 | `src/pages/launch.astro` のみ 5 行変更（テスト数は 1486 で不変）。コミット1本（`66be3cb`）。#311 / PR #328 がサマリ側 5 箇所を「最大6本」に直したときに残したカテゴリ別カード 4 箇所を「毎号 最大 N 本掲載」にし、CONTENTS の説明文に編集方針（「掲載基準を満たすタイトルが無い週は、枠を埋めずに本数を絞ります」）を追記した。**Issue 本文（自分で起票）より対象が広く、同じカード内の `content-card-count` 4 箇所を見落としていた**（合計 8 箇所）。後者は `opacity: 0.08` の装飾ウォーターマークなので意図的に対象外。1440px / 390px で実表示を確認（スクリーンショットは gitignore 対象の `screenshots/`） |

| **#336 対応** | `fix/issue-336-newrelease-prompt-framing` | ✅ **マージ済み**（2026-08-14。マージコミット `8dbbe24`。通常マージ、squashではない） | **#336**（Closed）/ 関連 **#163**（本 Issue の出所。同日クローズ）/ PR #337 | 33ファイル / **1495テスト**（着手前 33 / 1486。新規9件）。コミット2本（実装 `c0528a7` → `/code-review` 指摘対応 `263a0fb`）。**§8 計画表の外**（Open Issue 棚卸しで #163 から切り出した残り火）。`buildNewReleaseSystemPrompt` の冒頭 1 行を「大手ゲーム企業の新作ゲームを紹介する」→「注目の新作ゲームを紹介する」に変更。**論点A の実装（PR #237、2026-08-09）からプロンプトの追随が 5 日間漏れていた**もので、仕様 §2.1「企業規模は問わない」と正面から矛盾していた。この行は §2.5 の分岐より前にあるため**全発売状態に効いていた**。**ゲート撤廃後に生成された号はまだ無く**（vol.019 は 2026-08-07 発行）、次の定期実行が初回露出だったため当日中にマージした。Issue の対処案2（企業規模の推測を禁じる専用ルール）は**入れない判断**（既存の「提供データに無い情報は書かない」等が覆っており、規模を語らせない目的で「大手」をプロンプトに残すのは逆効果。理由は JSDoc に記録）。併せて陳腐化コメント 7 箇所を修正（挙動不変）。ミュータント検証 **6 種すべて検出**。`/code-review` は 5 エージェント中 4 つが指摘なし、1 件（採点 75。閾値未満のため PR コメントは投稿されず）を**管理者判断で採用**——`pickNewReleaseLabelCompany` は両方が静的リスト外なら `developer` を返すため「小規模スタジオ名はラベルにならない」という残存コメントがコードと矛盾していた |

| **#26 対応** | `feat/issue-26-early-access-state` | ✅ **マージ済み**（2026-08-14。マージコミット `f544886`。通常マージ、squashではない） | **#26**（Closed）/ 分離 **#339** / PR #340 | 34ファイル / **1566テスト**（着手前 33 / 1495。新規71件・**新規テストファイル `steam-utils.test.ts`**）。コミット1本（`5aec85b`。`/code-review` は**5エージェントすべて指摘ゼロ**だったため追加コミットは無い）。**§8 計画表の外**（棚卸しで「高」に分類されていた唯一の Issue）。仕様に §2.9 を新設し、早期アクセスを**発売状態（§2.8 の 3 値）と直交する軸**として追加した。判定の一次ソースは **Steam `appdetails` の `genres` id 70**（`l=japanese` で「早期アクセス」）のみで、**既に全 Steam 候補で呼んでいる API なので追加コストはゼロ**。IGDB `game_status` は `igdbGameStatus` として別に持ち、**名作枠の除外ゲートだけ**に使う（実測で両方向に外れるため。詳細は申し送り）。バリデータは独立判定項 `earlyAccessStatementIssues`（2 型）で **`warning` 止まり**（`error` にすると号ごとに Issue が自動起票される。#222 の `unrecognizedScreeningResponses` と同じ判断）。**実測した実害**: 公開 19 号 107 記事のうち Steam が EA と示すのは **8 タイトル / 10 記事**、うち 5 記事が「発売中」と断定・1 記事（vol.008『ARK: Survival Ascended』）は EA に一言も触れていなかった。⚠️ **PR #340 の本文・コミットメッセージ・仕様 §2.9 の初版は「11 記事」と書いていたが誤り**（本 docs PR で訂正した。数え直しの内訳は下記「実施結果」節）。**さらに今週の選定 5 本のうち 2 本が EA** で、修正が次号で即座に効く状態だった。ミュータント検証 **23 種すべて検出**だが、**うち 1 種は初回に生存**し「`needsCompletion` に足した条件が現状は死んでいる」ことを正しく指した（純関数 `needsStorefrontCompletion` に切り出して解消）。**Issue の原因推測②（発売日の 1 日ズレ）は一次ソースで再現せず**、③（プラットフォーム別発売日）は **#339** に分離した |

| **公開号の削除** | `chore/delete-vol-017-019` | ✅ **マージ済み**（2026-08-14。マージコミット `0a0709b`。通常マージ） | 関連 **#206** / **#211**（同日クローズ。「公開記事はそのまま残す判断」が結果的に覆った）/ PR #343 | **ユーザー判断による公開号の削除**（Issue に紐づかない運用作業）。理由は新作紹介の本数不足（017 = 0 本 / 018 = 1 本（DLC）/ 019 = 0 本。仕様は 2 本）。`history.json` 120 → 98 件、featureImage 3 枚、記事 3 本を削除。**号番号 17 は再利用する判断**（ユーザー確認済み）。`data/validation/` のレポートは意図的に残した。`/code-review` 指摘 1 件（spec / spec-review 側の引用が未注記）を採用し同 PR で対応 |
| **本番テスト実行** | （ブランチなし。`workflow_dispatch`） | ✅ **実行成功**（2026-08-14 10:23〜10:36 UTC。run 31792016284。CI 全ステップ成功・Cloudflare デプロイ済み） | 発見 **#346** / **#349** / **#350**、修正 PR #345 / #347 / #351 | **`publish_date=2026-08-08` を指定して本番パイプラインを検証した**（ユーザー判断）。結果: **6 本・4 カテゴリ充足（newRelease 2 本）は本番初**で、#237 の効果を本番で初確認。`articleCountShortfalls` は 0（#311 の初稼働で誤検出なし）、`earlyAccessStatementIssues` は 0（#26 が EA 終了作品 2 本を正しく扱い偽陽性なし）。#323 のズレは踏まなかった（選定 4 本すべて発売日 ≤ 8/8）。**同時に運用レイヤの欠陥 4 件を露出させた**（下記申し送り） |
| **ワークフロー修正** | `fix/workflow-report-selection` | ✅ **マージ済み**（2026-08-14。マージコミット `08bbc81`） | Issue 無し（本番実測で発見）/ `Refs #202` / PR #345 | 3 件を 1 PR で修正: ①レポート選択を辞書順末尾から**現在号固定**へ（4 箇所）②起票タイトルに発行日を入れて**号番号再利用時の衝突**を回避（#206 と条件付き衝突していた）③タイトルの要因内訳に Web 検索失敗を追加。**Sonnet の初回実装が `printf "%03d" 017` の 8 進数バグを踏み、管理者レビューで検出**（`10#` に修正）。`/code-review` 3 件採用（dedup コメントの矛盾・コメントの日付誤り・**LOUD FAILURE の位置**＝commit & push の後だったため split-brain になる箇所を前倒し） |
| **#346 対応** | `fix/issue-346-jp-url-query-fallthrough` | ✅ **マージ済み**（2026-08-14。マージコミット `7f3e851`） | **#346**（Closed）/ PR #347 | 34ファイル / **1579テスト**（着手前 1566。新規13件）。候補収集の打ち切り条件を「候補が 0 件なら次へ」から「**使える候補が得られなければ次へ**」に変更（選別 null / 検証 mismatch でフォールスルー。`uncertain` は従来どおり採用）。**ライブ API で修正確認**: `ほの暮しの庭` が query[1] で `nippon1.jp/consumer/honogurashi` を取得、`パルワールド` はハッピーパスで検索・選別・検証各 1 回（コスト不変）。`/code-review` は採点 80 以上ゼロだが 4 件を管理者判断で採用（削除された rationale コメントの復元・#135 P2-1 との精度トレードオフの明記・同名衝突の回帰テスト・PR 本文の推測記述を実測に差し替え） |
| **テスト号の削除** | `chore/delete-test-vol-017` | ✅ **マージ済み**（2026-08-14。マージコミット `eb9c3f3`） | 注記 **#206** / **#339** / PR #348 | 本番テスト実行が作った Vol.17（発行日 2026-08-08）を削除し、**クールダウンを解放**（`history.json` 108 → 98 件）。`data/validation/validation-report-017.*` を **`0a0709b` から復元**（テスト実行が上書きしていた）。**レポートのパスは号番号から生成されるため同じ号の再生成で上書きされる**ことを doc 3 ファイルと Issue 2 件の注記に追記した |
| **#349 対応** | `fix/issue-349-page-content-failure-severity` | ✅ **マージ済み**（2026-08-14。マージコミット `225a755`） | **#349**（Closed）/ 関連 **#350** / PR #351 | 34ファイル / **1596テスト**（着手前 1579）。`pageContentFailures` を `error` → `warning` に分離（`searchFailures` は `error` のまま）。実測根拠: `searchFailures > 0` は **0 / 12 号**、`pageContentFailures > 0` は **3 / 12 号**、うち単独で error にしたのは 2 件。**実データ検証**: 本日のテスト実行のレポート（`git show 054b9ba:...`）が `error` → `warning` に変わることを確認。`/code-review` **3 件が採点 80 以上**（`searchFailureCount` の JSDoc が逆の判断の根拠を引用 / `shouldFileIssue` の JSDoc が分離前の表記 / ワークフローのコメントが修正前の挙動を現在形で記述）で全件修正。⚠️ **管理者が #222 の教訓（未計測と 0 件を潰すな）を自分で踏み、既存の回帰テストが検出した** |
...
| 6 | 特集がイベント 0 件週に「人気順の先頭」へ収束（重複 50%。§4.3） | ✅ **#308 / #310 / PR #326**（2026-08-13） | 生成 |
...
| 9 | 本数不足に気づけない（vol.13 の名作 0 本が今回初めて判明。§6.4） | ✅ **#311 / PR #328**（2026-08-13） | 検証 |

**完了した 1〜5 はすべて「選定ロジック」、残った 6〜9 はすべて「記事生成・検証」。**
直近 3 日の派生作業は全部が**既に完了した選定ロジック側の深掘り**で、読者が実際に読む文章の事実性には触れていない。

> **2026-08-13 更新**: 6〜9 のうち **7（#309）/ 8（#307）/ 6（#310）/ 9（#311）がすべて完了**し、
> **仕様書が修正対象に挙げた故障 9 件は 9/9 が解消済み**になった。
>
> ⚠️ **ただし「解消」の意味は 9 件で一様ではない。** 1〜5 と 6〜8 は原因を直したが、
> **9（本数不足）は「気づける状態にした」だけで本数そのものは増えていない**（§6.4 が明記した設計判断）。
> 実測で直近 5 号のうち 4 号が不足しているので、**次に不足が出た号では Issue が自動起票される**。
> そこで初めて「埋めるかどうか」の判断材料（どの段階で候補が落ちたか）が集まる。
...
| ~~4~~ | ~~**#308**（PR-F0。発行日の JST 化）~~ | ✅ **完了**（2026-08-13。PR #322。マージ `713baa5`）。**#310 の測定前提だった**。生成側で JST に変換する案を採用。実測で**イベント件数が変わる週は 52 週中 29 週**あり、**イベント 0 件週は土曜基準 5 週 / 金曜基準 3 週**（→ #310 は土曜基準で測る）。PR-C の申し送り「`buildIndieCandidates` が `publishDate` を使っていない」は**層が違ったので #323 に分離**、`fetch-japanese-events.ts` の TZ 依存は **#324** に分離 |
| ~~5~~ | ~~**#310**（PR-F。特集のイベント0件週フォールバック）~~ | ✅ **完了**（2026-08-13。PR #326。マージ `6bd8041`）。土曜基準のイベント 0 件週 **5 週**を #308 マージ後の main で再現し、5 週すべてが過去方向 **1〜5 日**で埋まることを実データで確認。**通常週 47 週の挙動は不変**（テストで固定）。下位判断は記録先 = `history.json` の `featureEvents` / 除外号数 **N=2**。**#324 の TZ 依存も同 PR で解消してクローズ**（`TZ=America/Los_Angeles` で 0 件週が 5 → 3 週に変わる実害を実測） |
| ~~1~~ | ~~**#311**（PR-G。本数不足の検出）~~ | ✅ **完了**（2026-08-13。PR #328。マージ `6567ed1`）。**§8 計画表の最後の 1 行で、これで 15/15 完了。** 着手前に §9.1 保留1 を確認し、**high 警告に混ぜず独立した判定項にする**で決着（保留1 の結論に依存しなくなった）。実測で不足は「17 号中 3 号」ではなく **19 号中 5 号**、かつ**予測された実害は起票の 3 日前の vol.019 で既に発生していた**（`status=warning` で Issue 未起票） |

> ✅ **この表は 2026-08-13 に全項目が完了した。** 以後、優先順位を決める対象は `monitoring` の
> Issue だけになる（下記の表）。**新しい作業を足すときは、まず「Milestone に昇格する 2 条件」
> （上記「進め方」節）を満たすかを確認すること。** 満たさないものを積むと、この表が漂流の原因に戻る。
...
**この表には性格の違う 2 種類が入っている。混同しないこと。**

- **7 件（#239 / #240 / #251 / #285 / #317 / #318 / #323）= 未測定型。** Issue 自身が「実害の頻度が未測定」と記録し、**着手の前提として測定を要求している**もの。ラベルは 2026-08-13 に付与
- **3 件（#227 / #284 / #339）= 測定済み型。** 実測を済ませたうえで「実害が小さい」または「先に決める仕様判断がある」と判断して付与した（#227 / #284 は 2026-08-14 の棚卸し、#339 は同日の分類セッション）。**着手判断のための再測定は要らない**——必要なのは下表の再開条件が満たされたかの観測だけである
  - ただし **#339 だけは理由が「実害が小さい」ではない。** 断定と矛盾する記載は実測 9 件あり、うち **2 件は確定日が後発 = 明確な誤り**である。着手しないのは、**対処案 1〜3 のどれも「IGDB に日付が無い機種（9 件中 7 件）をどう扱うか」の仕様判断を先に要する**ため（日付が無いことは未発売を意味しない）。この判断はユーザーに確認する事項で、実装者が決めてよいものではない

| Issue | 再開条件 |
|---|---|
| **#227**（`getAppDetails` の取得失敗が候補欠落に直結） | ①`appdetails` で **HTTP 429 / 5xx が 1 件でも観測された**とき ②`type=unknown` のスキップログが**日本配信されているタイトル**に出たとき ③`cc=jp` を使う設計判断を仕様に明文化する作業に着手するとき。⚠️ **2026-08-14 の実測で Issue の対処案 (a) は的を外していると判明**（45 件すべて HTTP 200 / 429 は 0 件。欠落 2 件はいずれも `success:false` で、うち 1 件は日本ストア非配信の `War Thunder`）。**先に「日本非配信の除外は意図的か」を決める** |
| **#284**（単一トークン社名が `developer` 未設定を招く） | ①**#311 の `articleCountShortfalls` が発火した号で、落選理由に `still-missing-required` が並んだ**とき ②単一トークン社名のスタジオが公開記事で開発元欄を落としているのが観測されたとき ③`isQualifiedCompanyName` を触る別の作業が発生したとき。⚠️ **ローカル `data/aggregated.json` では測れない**（`steamRawDeveloper` が 0 件 = 検出力ゼロ）。**ゲートには真陽性がある**（`ItsRave_n` / `lemorion_1224`）ので単純撤廃は不可 |
| **#339**（プラットフォーム別発売日を表現できない） | ①**本番レポート**（`data/validation/`）の `llm-judge-contradicted` に「**記載した機種が未発売**」型が 1 件でも出たとき（dev レポートでの発火は数えない。下記の方向内訳を参照） ②IGDB クエリに `release_dates.platform` を足す別作業が発生したとき（材料の取得コストが 0 になる） ③「対応機種は発売済みのみ記載する」等の運用方針が決まったとき。⚠️ **着手前にユーザーの仕様判断が 1 件必要**: IGDB に日付が無い機種（実測 9 件中 **7 件**）を「未発売」と扱うか「不明」と扱うか。⚠️ **実測の内訳**: 公開 19 号 / IGDB 解決できた 80 記事のうち断定と矛盾する記載は **9 件**、うち**確定日が後発 = 明確な誤りは 2 件**（vol.011 `LEGO Batman` / vol.018 `Street Fighter 6` の Switch 2）。judge が拾えているのは **1 件**（dev-011 の `Forza Horizon 6`。確信度 90%） |
...
| **#323**（選定が `PUBLISH_DATE` を尊重しない） | ①過去日を指定した再生成で、選定結果が当時と食い違って**読者に見える問題**が起きたとき ②`fetch-data` の選定基準日を `PUBLISH_DATE` に揃える運用方針が決まったとき。⚠️ **これは「JST かどうか」の問題ではない**（選定側は PR-C で既に JST 基準）。「今のデータで作り直す」のが意図なら仕様どおりなので、**先に運用意図を決める**必要がある |
| ~~**#324**~~（`isEventOnDate` の TZ 依存） | ✅ **解消・クローズ済み**（2026-08-13。PR #326）。解除条件①「#310 に着手するとき」を満たしたので同 PR で修正した。**実測した実害**: `TZ=America/Los_Angeles` でイベント 0 件週が 5 週 → 3 週に変わり、記念日の日付が全体に 1 日ずれる。日付解釈を `getJstDateString` 基準に統一し、日付加減算も date-fns（ローカル時刻ベース）から UTC ミリ秒演算に変更。5 TZ（JST / UTC / LA / ホノルル / キリバス+14）で同一結果になることをテストで固定 |

### 2026-08-14 の Open Issue 棚卸し（全 33 件）

キューが空になったため、**「着手すべきものがあるか」を Open Issue 全件の本文とコードの突き合わせで確認した**。分類は「対応不要 / 高 / 中 / 低」。

| 分類 | 件数 | 内訳 |
|---|---|---|
| **対応不要（クローズ済み）** | **5** | #38（対応案 1・2 とも実装済み）/ #163（論点A で決着・実装済み。残り火は #336 へ）/ #175（完了条件を PR #237 が充足）/ #206・#211（原因は既修正。公開記事はそのまま残す判断） |
| **高** | 2 → **1** → **0** | ~~#26~~（早期アクセスの概念がコードに無い。`grep` で実装 0 件）→ ✅ **完了・クローズ**（2026-08-14。PR #340）/ ~~#284~~（→ 実測後に**中**へ格下げ） |
| **中** | 9 → **10** | #74 / #83 / #156 / #159 / #212 / #213 / #236 / #293 / **#227**（実測後に `monitoring`）/ **#284**（高から格下げ・`monitoring`） |
| **低** | 17 | #66 / #155 / #176 / #177 / #178 / #187 / #203 / #210 / #217 / #228 + `monitoring` 既存 7 件（#239 / #240 / #251 / #285 / #317 / #318 / #323） |

> 分類の合計は棚卸し時点の **33 件**（5 + 2 + 9 + 17）。矢印は本セッション中の実測による移動を示す。

**この棚卸しで確認した主な事実**（いずれも実測）:

| Issue | 確認した内容 |
|---|---|
| #26 | `earlyAccess` / `early_access` / `早期アクセス` の実装ヒットは **0 件**。#309 の 3 値化は「発売済み / 本日発売 / 発売予定」だけで Early Access は別軸のまま。かつプロンプトが「発売中と記載し、『発売予定』とは絶対に書かない」と**断定を強制**している（`bedrock-client.ts`）。→ ✅ **2026-08-14 に対応**（PR #340。仕様 §2.9 新設）。**この棚卸しは「概念が無い」ことは正しく捉えていたが、実害の件数を測っていなかった**（着手時に公開 19 号で 10 記事・今週の選定でも 2 本と判明） |
| #159 | 未修正の根本原因が 2 つ残る（`uncertain` の無条件採用 / 採用時の HEAD 確認なし）。ただし**本番 CI の失敗は 2026-07-03 の 1 回だけで、以降 7 回連続 success**（差し替え機構 #179 slotGates が後から整備されたため）→ 優先度は中 |
| #187 | Issue の主張「大手枠混入」は**論点A の企業規模ゲート撤廃で陳腐化**。かつ `fetch-data.ts:1448`（zombie）→ `:1458`（completeness gate）の順で補充分も R0–R5 で再検証される → 優先度は低 |
| #217 | Issue が挙げた閾値の反転バグは **#235 で経路ごと削除され消滅**（`POPULARITY_YOUTUBE_PERCENTILE` はヒット 0）。ただし **YouTube API の取得自体は継続中**（`fetch-data.ts:1383`）で、データは記事コンテキストにしか使われていない |
| #83 | 実測: `bedrock-client.ts:17` = `global.anthropic.claude-sonnet-4-5-20250929-v1:0` / `generate-feature-image.ts:24` = `amazon.nova-canvas-v1:0`。**非推奨・使用不可のモデルは使っていない** |
| #293 | 仕様書を `CERO` / `レーティング` / `ESRB` / `年齢制限` で grep して **0 件**を再確認。コード変更ではなく**判断の記録**が必要な案件 |
| #177 | 重複 3 箇所すべて現存（`fetch-data.ts:65` / `finalize-game-metadata.ts:60` / `validate-article.ts:692`） |
| #203 | `clean` 系 npm script は未整備。実測サイズは `features-dev` 42M / `screenshots` 38M / `.playwright-mcp` 7.9M |
| #176 | `known-cases.json` は **7 件**（#297 で 1 件増）|

> 💡 **副産物: #311 の本数不足検出が、他 Issue の実害測定手段になった。** #284 / #227 はどちらも「本数不足の原因候補」として測定したもので、単発測定では主要因と言えなかった。**次に `articleCountShortfalls` が発火した号で落選理由を見るのが、両者に共通する最短の検証経路**。

### 棚卸し後に起票された分の分類（2026-08-14）

上の棚卸し表は **33 件のスナップショット**なので、それ以降に起票された Issue は表に載らない。該当は **4 件**で、いずれも起票と同じセッションで分類した（下記「再発防止」の 3 点セットに従う）。

| Issue | 出所 | 分類 | 措置 |
|---|---|---|---|
| **#339**（記事が挙げた対応機種の一部が発行日時点で未発売） | #26 の原因推測③から分離（2026-08-14） | **中** | `monitoring` を付与し、上表に再開条件を記入。**Milestone には入れない** |
| ~~**#346**~~（日本語公式が実在しても拾えない） | 本番テスト実行で発見（2026-08-14） | **高** | ✅ **即日修正・クローズ**（PR #347）。読者に見えるリンクが英語になる実害が観測できたため、分類より先に修正した |
| ~~**#349**~~（`pageContentFailures` を error に昇格させている） | 本番テスト実行で発見（2026-08-14） | **中** | ✅ **即日修正・クローズ**（PR #351）。次の定期実行で誤起票が起きる状態だったため即日対応 |
| **#350**（チェック機構の重大度設計と運用アクションが対応していない） | 本番テスト実行の観測から、#156 の下位として切り出し（2026-08-14） | **中** | **`monitoring` は付けない**（観測待ちではなく、ユーザーの設計判断待ちのため）。**次に着手する 1 件**として現在地の表に記載 |

> 💡 **#346 / #349 は「分類して待つ」より「即日直す」が正しかった。** どちらも**次の定期実行で読者に見える結果が変わる**状態で、かつ修正が小さかった。**分類の枠組みは「着手すべきか判断に迷うもの」のためにあり、迷わないものに適用すると遅れるだけである。**
>
> **#350 に `monitoring` を付けなかった理由**: `monitoring` は「再開条件として観測を要求する」ラベルである（上表の 10 件はすべてそう）。#350 の引き金は観測ではなく**ユーザーとの設計判断**なので、`monitoring` に落とすと「観測待ち」と誤読され、永久に開かれない。#339 は仕様判断待ちだが実害の頻度も小さいので `monitoring` に置いた——**この 2 件の違いは「今すぐ判断する価値があるか」である**

**Milestone に昇格させなかった判断の根拠**（昇格の 2 条件は「進め方」節）:

- 条件①（Milestone の項目をブロックする）は**満たさない**——Milestone の 5 件はすべて CLOSED で、ブロック対象が存在しない
- 条件②（読者に見える実害が観測された）は**形式的には満たす**（確定日が後発の明確な誤りが 2 件）。それでも昇格させなかったのは、**対処案 1〜3 のどれもユーザーの仕様判断（日付が無い機種の扱い）を先に要し、着手可能な状態に無い**ため。前例は **#284**（実測 1 件の読者に見える実害があるが `monitoring`）
- したがって **#339 の再開は「観測」ではなく「仕様判断」が引き金**になる。上表の再開条件③がそれに当たる
...
**🚨 Issue を分離起票したら、同じセッションで分類まで終える（2026-08-14 に追加）。**
「進め方」節の「途中: 新しい欠陥を見つけたら Issue 化し、Milestone には入れず本線に戻る」は、
**起票だけで手を離すと「ラベルも分類も無い Open Issue」を作る**（#339 で実際に起きた）。
起票時に次の 3 つをセットで済ませること。**本線に戻るのはその後**。

1. **昇格の 2 条件を満たすか**を判定する（満たさなければ `monitoring`）
2. **ラベルを付ける**（GitHub 上の可視性。doc の記述はラベルの代わりにならない）
3. **再開条件を書く**（`monitoring` 表の行。「何を観測したら開けるか」が無い Issue は永久に開かない）

なお本件は、前セッションが 🚨 付きで同種の注意（棚卸しで分類を動かしたら現在地の表も直す）を
書いた**直後に再発**した。**注意書きの追加だけでは再発を止められない**ので、上の 3 点は
「起票」という 1 つの動作の定義に含める形で書いてある。
...
`getJstDateString` **:45** / `getJstDayStartUnixSec` **:68**（→ **PR #322 で +1 ずれた**。下記）。

⚠️ **さらに PR #322（Issue #308 / PR-F0。マージ `713baa5`）で `jst-date.ts` と `generate-articles.ts` の行番号がずれた。ずれ幅は 2 段階。**
`jst-date.ts` はファイル冒頭の説明に 1 行足したため**既存シンボルが一律 +1**、`generate-articles.ts` は
import を 1 行足したため **`:47` 以降が +1**、さらに発行日ブロックのコメントが 1 行 → 4 行になったため
**そのブロック以降が +4** になる。マージ後の実測値（2026-08-13。`5dbb6e6` → `713baa5`）:

| シンボル（ファイル） | PR #322 前 | PR #322 後 | ずれ |
|---|---|---|---|
| `JST_OFFSET_MS`（`jst-date.ts`） | :31 | **:32** | +1 |
| `getJstDateString`（`jst-date.ts`） | :45 | **:46** | +1 |
| `getJstDayStartUnixSec`（`jst-date.ts`） | :68 | **:69** | +1 |
| `PUBLISH_DATE_PATTERN`（`jst-date.ts`。本PRで新設） | — | **:76** | 新設 |
| `resolvePublishDateString`（`jst-date.ts`。本PRで新設） | — | **:110** | 新設 |
| `import { resolvePublishDateString }`（`generate-articles.ts`） | — | **:47** | 新設 |
| `_releaseStatus`（`generate-articles.ts`） | :201 | **:202** | +1 |
| `generateNewReleaseArticle`（`generate-articles.ts`） | :354 | **:355** | +1 |
| `getEventsInRange(publishDate, 7)` の呼び出し（`generate-articles.ts`） | :749 | **:750** | +1 |
| **発行日の決定行**（`generate-articles.ts`） | :1309 | **:1313** | **+4** |
| `publishDate: publishDateStr`（`generate-articles.ts`） | :1496 | **:1500** | +4 |

⚠️ **さらに PR #326（Issue #310 / PR-F。マージ `6bd8041`）で 5 ファイルの行番号がずれ、`scripts/fetch-japanese-events.test.ts` が新設された。ずれ幅はファイルごとに段階的で、一律のオフセットは当たらない。**
マージ後の実測値（2026-08-13。`f5c7ccb` → `6bd8041`。`git show <rev>:scripts/<file>` を grep して測定）:

**`fetch-japanese-events.ts`（192 行 → 380 行。ほぼ全面改訂）**

| シンボル | PR #326 前 | PR #326 後 | ずれ |
|---|---|---|---|
| `JapaneseEvent` / `JapaneseEventsData` / `ResolvedEvent` | :11 / :21 / :26 | **:24** / **:34** / **:39** | +13 |
| `DEFAULT_FEATURE_EVENT_FORWARD_WINDOW_DAYS` / `..._BACKWARD_MAX_DAYS` / `..._EXTENDED_FORWARD_MAX_DAYS`（本PRで新設） | — | **:53** / **:59** / **:68** | 新設 |
| `readFeatureEventForwardWindowDays` / `...BackwardMaxDays` / `...ExtendedForwardMaxDays`（新設） | — | **:85** / **:93** / **:101** | 新設 |
| `loadJapaneseEvents` | :37 | **:111** | +74 |
| `getJstDateParts` / `addUtcDays`（新設。#324 の修正の中核） | — | **:127** / **:133** | 新設 |
| `getNthWeekdayOfMonth`（**UTC 0 時の Date を返すよう変更**） | :56 | **:148** | +92 |
| `isEventOnDate`（**JST 解釈に変更**） | :84 | **:168** | +84 |
| `getEventsInRange`（**第3引数 `eventsData` を追加**） | :128 | **:212** | +84 |
| `FeatureEventSource` / `FeatureEventSelection` / `SelectFeatureEventOptions`（新設） | — | **:245** / **:247** / **:260** | 新設 |
| `selectFeatureEventCandidates`（**本PRの中核。新設**） | — | **:296** | 新設 |
| `debugShowMonthEvents` | :156 | **:345** | +189 |

**`game-history.ts`**

| シンボル | PR #326 前 | PR #326 後 | ずれ |
|---|---|---|---|
| `HISTORY_PATH` / `COOLDOWN_WEEKS` / `HistoryEntry` | :14 / :19 / :28 | **:21** / **:26** / **:35** | +7 |
| `FeatureEventHistoryEntry`（新設） | — | **:51** | 新設 |
| `DEFAULT_FEATURE_EVENT_EXCLUDE_ISSUE_COUNT` / `readFeatureEventExcludeIssueCount`（新設） | — | **:75** / **:81** | 新設 |
| `loadHistory` | :44 | **:92** | +48 |
| `saveHistory`（**第2引数 `newFeatureEvents` を追加**） | :60 | **:112** | +52 |
| `getCooldownTitles` | :99 | **:176** | +77 |
| `getRecentFeatureEventNames`（新設） | — | **:213** | 新設 |
| `getHistoryPath` / `createHistoryEntry` | :128 / :135 | **:236** / **:243** | +108 |
| `createFeatureEventHistoryEntry`（新設） | — | **:264** | 新設 |

**`bedrock-client.ts`（`featureThemeSelectionPrompt` までは 0。それ以降が +59 → +62）**

| シンボル | PR #326 前 | PR #326 後 | ずれ |
|---|---|---|---|
| `getReleaseStatus` / `PromptTemplates` / `buildUserMessage` / `buildFeatureUserMessage` | :128 / :302 / :536 / :646 | :128 / :302 / :536 / :646 | **0** |
| `featureThemeSelectionPrompt` | :712 | :712 | **0** |
| `FeatureThemeSelection`（新設） | — | **:739** | 新設 |
| `matchSelectedEventName`（新設） | — | **:765** | 新設 |
| `selectFeatureThemeWithAI`（**戻り値を `{ theme, selectedEventName }` に変更**） | :739 | **:798** | +59 |
| 固定文言 `'今週の注目ゲーム特集'`（フォールバック本体） | :743 | **:802** | +59 |
| `proposeThemeGamesFromKnowledge` | :825 | **:887** | +62 |
| `prefilterFeatureCandidatesByTheme` / `selectFeatureGames` | :995 / :1058 | **:1057** / **:1120** | +62 |
| `determineFeatureTheme`（**呼び出し元ゼロのまま。未削除**） | :1125 | **:1187** | +62 |
| 固定文言 `'今週の注目ゲーム特集'`（`determineFeatureTheme` 内。死んだコード） | :1173 | **:1235** | +62 |
| `parseArticleResponse` / `parseTitleResponse` | :1179 / :1199 | **:1241** / **:1261** | +62 |

**`generate-articles.ts`（+1 → +21/+22 → +24/+25 → +54 → +64 の 5 段階）**

| シンボル | PR #326 前 | PR #326 後 | ずれ |
|---|---|---|---|
| `getNextIssueNumber` / `WebSearchSource` / `RegenerateOptions` | :86 / :111 / :125 | **:87** / **:112** / **:126** | +1 |
| `FeatureEventInfo`（新設） | — | **:146** | 新設 |
| `GeneratedArticle`（`featureEvent?` を追加） | :134 | **:155** | +21 |
| `GeneratedIssue` | :160 | **:182** | +22 |
| `_releaseStatus` | :202 | **:224** | +22 |
| `deduplicateGames` | :670 | **:692** | +22 |
| `buildFeatureArticleFromContext`（**`__test` に追加して単体テスト可能にした**） | :702 | **:726** | +24 |
| `generateFeatureArticle`（**第6引数 `excludeEventNames` を追加**） | :742 | **:767** | +25 |
| イベント探索の呼び出し（**`getEventsInRange(publishDate, 7)` → `selectFeatureEventCandidates(publishDate, ...)`**） | :750 | **:779** | 関数ごと変更 |
| `generateClassicArticle` / `createFallbackGame` | :1115 / :1229 | **:1169** / **:1283** | +54 |
| **発行日の決定行**（`resolvePublishDateString`） | :1313 | **:1367** | +54 |
| `getCooldownTitles('feature')` の呼び出し | :1410 | **:1464** | +54 |
| `publishDate: publishDateStr` / `__test` | :1500 / :1560 | **:1564** / **:1624** | +64 |

**`build-issue.ts`（+1 → +45 → +51）**

| シンボル | PR #326 前 | PR #326 後 | ずれ |
|---|---|---|---|
| `getNextIssueNumber` / `isCriticallyIncompleteArticle` | :76 / :161 | **:77** / **:162** | +1 |
| `collectFeatureEventHistoryEntries`（新設。テスト可能にするため main() から切り出した） | — | **:181** | 新設 |
| `formatArticleForFrontmatter` | :174 | **:219** | +45 |
| `escapeYamlString` / `generateArticleContent` | :359 / :424 | **:404** / **:469** | +45 |
| 履歴更新ブロック（`Updating game history`） | :623 | **:668** | +45 |
| `saveHistory` の呼び出し | :654 | **:705** | +51 |

**特筆すべき点**: `generate-articles.ts` のずれ幅は **+1 → +21 → +22 → +24 → +25 → +54 → +64** と 7 段階に分かれる
（型定義の追加・特集記事フローの追記・main() への除外リスト構築の追加がそれぞれ別の位置にあるため）。
`bedrock-client.ts` は **`featureThemeSelectionPrompt`（:712）までずれ 0** で、それ以降だけがずれる。
**固定文言 `'今週の注目ゲーム特集'` の 2 箇所（:802 / :1235）はどちらも残っている**——:802 は
「どの探索段階でも記念日が見つからなかったとき」の最後の砦として意図的に維持（実データの 2026 年では到達しない）、
:1235 は `determineFeatureTheme` 内の死んだコードで本PRのスコープ外。

新設ファイル `scripts/fetch-japanese-events.test.ts`（37 テスト）の主要 describe:
`既定値` / `getNthWeekdayOfMonth` / `getEventsInRange — 探索窓の境界` /
`タイムゾーン非依存（Issue #324）` / `selectFeatureEventCandidates — 通常週` /
`— 0 件週の過去方向フォールバック` / `— 過去方向の境界値（ちょうど 7 日前 / 8 日前）` /
`— 拡張未来方向（8 日目以降）` / `— 使用済み記念日の除外` / `loadJapaneseEvents — 実データの前提`。

**特筆すべき点**: `generate-articles.ts` は**発行日ブロックの前後でずれ幅が +1 → +4 に変わる**。
一律 +1 を当てると `:1313` 以降が 3 行ずれる。`jst-date.ts` は新設シンボルが**既存 2 関数より後ろ**にあるので、
`getJstDateString` / `getJstDayStartUnixSec` を探すときは +1 だけを見ればよい。
なお **PR #322 は `fetch-data.ts` / `fetch-igdb.ts` / `bedrock-client.ts` / `validate-article.ts` を触っていない**ので、
それらの行番号は PR #320 の表がそのまま有効。

**特筆すべき点**: `PromptTemplates.newReleaseSystem` は **`buildNewReleaseSystemPrompt('発売済み')` の呼び出しになった**ので、
プロンプト文面を探して `grep` しても `PromptTemplates` 付近には無い。文面は `buildNewReleaseSystemPrompt`（**PR #337 後は `:218`**。下記）の中にある。
`searchGameInfo` は**第4引数が `{ releaseYear?, releaseStatus? }` の options オブジェクトに変わった**ので、
過去の「第4引数に発売年を渡す」という記述はすべて無効。

⚠️ **さらに PR #337（Issue #336。マージ `8dbbe24`）で `bedrock-client.ts` と `generate-articles.ts` の行番号がずれた。**
`bedrock-client.ts` は `buildNewReleaseSystemPrompt` の JSDoc に 12 行足したため **その関数以降が一律 +12**、
それより前（`getReleaseStatus` / `isUpcomingForBody` / `QUANTITATIVE_TO_QUALITATIVE_RULE`）は **0**。
`generate-articles.ts` はラベル選定のコメントを 3 行 → 7 行にしたため **その位置以降が +4**、それより前は **0**。
マージ後の実測値（2026-08-14。`5c746c9` → `8dbbe24`。`git show <rev>:<file>` を grep して測定）:

| シンボル（ファイル） | PR #337 前 | PR #337 後 | ずれ |
|---|---|---|---|
| `getReleaseStatus`（`bedrock-client.ts`） | :128 | :128 | **0** |
| `isUpcomingForBody`（`bedrock-client.ts`） | :170 | :170 | **0** |
| `QUANTITATIVE_TO_QUALITATIVE_RULE`（`bedrock-client.ts`） | :179 | :179 | **0** |
| `buildNewReleaseSystemPrompt`（`bedrock-client.ts`） | :206 | **:218** | +12 |
| `PromptTemplates.newReleaseSystem` | :308 | **:320** | +12 |
| `PromptTemplates.indieSystem` / `featureSystem` | :313 / :380 | **:325** / **:392** | +12 |
| `PromptTemplates.classicSystem` / `titleSystem` | :430 / :488 | **:442** / **:500** | +12 |
| `buildUserMessage` / `buildFeatureUserMessage` | :536 / :646 | **:548** / **:658** | +12 |
| `featureThemeSelectionPrompt` / `selectFeatureThemeWithAI` | :712 / :798 | **:724** / **:810** | +12 |
| `prefilterFeatureCandidatesByTheme` / `parseArticleResponse` | :1057 / :1241 | **:1069** / **:1253** | +12 |
| `generateNewReleaseArticle`（`generate-articles.ts`） | :378 | :378 | **0** |
| `pickNewReleaseLabelCompany` の呼び出し（`generate-articles.ts`） | :448 | **:452** | +4 |
| **発行日の決定行**（`resolvePublishDateString`。`generate-articles.ts`） | :1368 | **:1372** | +4 |
| `getCooldownTitles('feature')` の呼び出し（`generate-articles.ts`） | :1465 | **:1469** | +4 |
| `publishDate: publishDateStr` / `__test`（`generate-articles.ts`） | :1565 / :1625 | **:1569** / **:1629** | +4 |
| `SelectedGames.newReleases`（`types.ts`） | :226 | :226 | **0**（コメントのみ変更） |
| `removeZombieGames`（`fetch-data.ts`） | :788 | :788 | **0**（コメントのみ変更） |
| `pickNewReleaseLabelCompany`（`indie-classifier.ts`） | :319 | :319 | **0**（コメントのみ変更） |

**特筆すべき点**: ファイル行数は `bedrock-client.ts` 1366 → 1378 行、`generate-articles.ts` 1638 → 1642 行。
**コメントだけを直した 3 ファイル（`types.ts` / `fetch-data.ts` / `indie-classifier.ts`）は行数が変わらないのでずれ 0** だが、
`generate-articles.ts` は**コメントの行数を増やしたためずれが出ている**。「コメント変更だからずれない」とは限らない。
なお **PR #337 は 発行日の決定行を触っていない**（`:1368` → `:1372` は上流のコメント増加による純粋な位置移動）。
⚠️ **PR #326 の表が記録していた `:1367`（発行日の決定行）/ `:1564`（`publishDate: publishDateStr`）は、本 PR の着手前に既に +1 ずれて `:1368` / `:1565` になっていた。**
原因は **PR #333（Issue #330）のコミット `f754882`** で、`import { isMainModule } from './entrypoint.js';` を 1 行足したためである
（実測: `f754882^` で `:1367` → `f754882` で `:1368`。`6bd8041..5c746c9` の範囲で `generate-articles.ts` を触ったコミットはこれだけ）。
**PR #333 の docs（本ファイルの #330 対応行）はこの副作用を記録していなかった**——`entrypoint.ts` の新設とガードの差し替えは記録したが、
**import 1 行がファイル全体を +1 ずらすことに触れていない**。この表の「PR #337 前」列はすべて実測値である。

⚠️ **さらに PR #340（Issue #26。マージ `f544886`）で 9 ファイルの行番号がずれた。ずれ幅はファイルごとに違い、同じファイル内でも位置によって違う。**
マージ後の実測値（2026-08-14。`dce0767` → `f544886`。`git show <rev>:<file> | grep -n` で全件測定）:

| シンボル（ファイル） | PR #340 前 | PR #340 後 | ずれ |
|---|---|---|---|
| `getReleaseStatus` / `isUpcomingForBody`（`bedrock-client.ts`） | :128 / :170 | :128 / :170 | **0** |
| `buildNewReleaseSystemPrompt`（`bedrock-client.ts`） | :218 | :218 | **0** |
| `PromptTemplates.newReleaseSystem` / `titleSystem` | :320 / :500 | :320 / :500 | **0**（`titleSystem` は**中身に 2 行足したが定義開始行は動かない**） |
| `buildUserMessage`（`bedrock-client.ts`） | :548 | **:582** | +34 |
| `buildFeatureUserMessage`（`bedrock-client.ts`） | :658 | **:711** | +53 |
| `aggregateGames`（`fetch-data.ts`） | :180 | **:210** | +30 |
| `deduplicateGames`（`fetch-data.ts`） | :542 | **:578** | +36 |
| `buildIndieCandidates`（`fetch-data.ts`） | :1130 | **:1171** | +41 |
| `buildClassicCandidates`（`fetch-data.ts`） | :1183 | **:1228** | +45 |
| `IGDB_GAME_FIELDS`（`fetch-igdb.ts`） | :412 | **:443** | +31 |
| `IGDB_POOL_QUERY_FIELDS`（`fetch-igdb.ts`） | :787 | **:819** | +32 |
| `fetchClassicGames` / `fetchIndieGames` / `fetchIGDBData`（`fetch-igdb.ts`） | :1117 / :1156 / :1190 | **:1152** / **:1191** / **:1225** | +35 |
| `validateUpcomingEvaluationClaims`（`validate-article.ts`） | :940 | **:969** | +29 |
| `validateArticle`（`validate-article.ts`） | :1008 | **:1037** | +29 |
| `detectArticleCountShortfalls`（`validate-article.ts`） | :1125 | **:1154** | +29 |
| `validateArticles`（`validate-article.ts`） | :1150 | **:1296** | **+146** |
| `writeAndCheckReport`（`validate-article.ts`） | :1213 | **:1361** | **+148** |
| **発行日の決定行**（`resolvePublishDateString`。`generate-articles.ts`） | :1372 | **:1392** | +20 |
| `__test`（`generate-articles.ts`） | :1629 | **:1649** | +20 |
| `GeneratedArticle` 型（`generate-articles.ts`） | :156 | :156 | **0** |
| `GameData` 型（`types.ts`） | :96 | **:108** | +12 |
| `IGDBGame` 型（`types.ts`） | :41 | :41 | **0** |
| `isFanGame`（`game-filter.ts`） | :70 | :70 | **0** |
| `isQualifiedCompanyName`（`steam-utils.ts`） | :35 | **:67** | +32 |
| `computeReportStatus`（`format-validation-report.ts`） | :88 | **:102** | +14 |
| `formatReportMarkdown`（`format-validation-report.ts`） | :223 | **:254** | +31 |

**新設したシンボルの現在位置**（PR #340 後の実測値）:

| シンボル | 位置 |
|---|---|
| `detectEarlyAccessFromSteamGenres` / `STEAM_GENRE_ID_EARLY_ACCESS` | `steam-utils.ts:45` / `:27` |
| `isEarlyAccessGame` | `game-filter.ts:93` |
| `needsStorefrontCompletion` | `fetch-data.ts:196` |
| `IGDB_GAME_STATUS_EARLY_ACCESS` / `buildNotEarlyAccessFilter` | `fetch-igdb.ts:418` / `:438` |
| `detectEarlyAccessStatementIssues` / `detectEarlyAccessStatementIssuesForIssue` | `validate-article.ts:1225` / `:1285` |
| `earlyAccessStatementIssueCount` | `format-validation-report.ts:88` |

**特筆すべき点**: ファイル行数は `validate-article.ts` 1380 → 1543（+163）、`bedrock-client.ts` 1378 → 1441（+63）、
`format-validation-report.ts` 357 → 416（+59）、`fetch-data.ts` 1542 → 1588（+46）、`fetch-igdb.ts` 1311 → 1346（+35）、
`steam-utils.ts` 112 → 144（+32）、`types.ts` 231 → 260（+29）、`generate-articles.ts` 1642 → 1662（+20）、
`game-filter.ts` 75 → 95（+20）。

⚠️ **`validate-article.ts` は同一ファイル内でずれ幅が +29 と +146 に割れている。** 新しい検出器
（163 行）を `validateArticles` の直前に入れたためで、**それより前のシンボル（`validateArticle` 含む）は +29 のまま**。
「ファイルの行数が +163 だから全部 +163」と考えると外す。**同じファイルでも挿入位置より前と後で分けて測ること。**

⚠️ **`PromptTemplates.titleSystem` は中身に 2 行足したのに定義開始行が動いていない。** テンプレート文字列の
内部に追記した場合、その定義自体の開始行は変わらず、**後続のシンボルだけがずれる**。「触ったファイルのシンボルは全部ずれる」わけではない。
...
## 実施結果（2026-08-13。PR #322。マージ `713baa5`）

### 0. 触ったファイル

| ファイル | 変更 |
|---|---|
| `scripts/jst-date.ts` | `resolvePublishDateString`（**:110**）と `PUBLISH_DATE_PATTERN`（**:76**）を新設（+67行）。ファイル冒頭の関数一覧にも 1 行追記 |
| `scripts/generate-articles.ts` | 発行日の決定行（旧 `:1309` → 新 **`:1313`**）を差し替え + import 1 行。**本体の変更はこの 1 行だけ** |
| `scripts/jst-date.test.ts` | 新規 27 件（`resolvePublishDateString` の describe 4 ブロック） |
| `.github/workflows/weekly-build.yml` | `Commit and push` のコミットメッセージの日付を `TZ=Asia/Tokyo date` に。**表記のみ**で発行日の決定には関与しない |

### 1. 着手前検証: Issue の原因説明は正しかったが、数値は 2 つ古かった

Issue #308 の原因説明（cron は正しい / `PUBLISH_DATE` が schedule 実行で空 / `toISOString()` が UTC 基準）は**コードで再確認して全て正しかった**。ただし独立に測り直すと 2 点ずれていた:

| 項目 | Issue / 仕様書の記述 | 実測（2026-08-13） |
|---|---|---|
| 金曜になっている号数 | 17 号中 11 号 | **19 号中 13 号**（`src/content/issues/*.md` の frontmatter 全件。土=6） |
| 発行日を決めている行 | `generate-articles.ts:1302` | **`:1309`**（PR #320 で +7 ずれていた） |

**土曜だった 6 号は vol.1 / 2 / 7 / 9 / 14 / 15。** 号番号が連続していないので、schedule 実行と手動実行が混在した結果と考えられる（確証はない）。

**発行日の決定点が 1 箇所であることも確認した。** `build-issue.ts:505` は `generated-issue.json` の `publishDate` を読み直すだけで、独自に「今日」を導出しない（`grep -rn "publishDate" scripts/` の全件確認）。したがって 1 箇所直せば下流すべてに効く。

### 2. 実装方針の選択（§9.3-2 が挙げた 2 案のうち後者）

| 案 | 採否 | 理由 |
|---|---|---|
| ワークフローで JST の日付を `PUBLISH_DATE` に渡す | ❌ | `weekly-build.yml` の schedule 経路しか直せない。ローカル実行・`build-issue:dev` は覆えない。**境界値テストがテストスイートの外に出る**（シェルの日付計算は `npm run test` の対象外） |
| **生成側で JST に変換する** | ✅ | 1 箇所で全経路を覆える。境界値をユニットテストで検証できる。**PR-C が新設した `getJstDateString` をそのまま再利用できる** |

### 3. 発行日が 1 日動くと何が変わるか（実測）

2026 年の全 52 週について cron 発火時刻（金曜 UTC 21:00）を再現し、修正前後で実 `getEventsInRange` を回した:

| 指標 | 値 |
|---|---|
| 発行日が変わる週 | **52 / 52 週**（全週が金 → 土に移る） |
| イベント**件数**が変わる週 | **29 / 52 週** |
| イベント 0 件週（金曜基準） | **3 週**（05-22 / 08-21 / 09-11） |
| イベント 0 件週（土曜基準） | **5 週**（04-11 / 05-23 / 06-13 / 08-22 / 09-12） |

**§9.3-2 が記録していた「土曜基準 5 週 / 金曜基準 3 週」を独立に再現した。** これで **PR-F（#310）の測定基準は土曜基準の 5 週で確定**する。⚠️ **#308 より前に測った 0 件週の数字はすべて金曜基準なので、#310 では main で測り直すこと。**

### 4. PR-C の不変条件を壊さないための制約

戻り値を**時刻を含まない `YYYY-MM-DD` に限定**した。呼び出し側の `new Date(str)` が **UTC 0 時**になることは PR-C の `getReleaseStatus`（`publishDate` を `getJstDateString` で JST カレンダー日付に落として比較する）の前提であり、時刻付きの値を返すとこの前提が崩れる。テストで固定した（「戻り値の形式」の describe 2 件）。

### 5. スコープに足した判断: 不正な `PUBLISH_DATE` を生成前に落とす

従来は `PUBLISH_DATE=2026/08/15` のような値が Invalid Date のまま伝播し、**Bedrock の生成コストを払い切った後**に `build-issue.ts` の `format()` が RangeError で落ちていた。新設関数の契約（`YYYY-MM-DD` を返す）を自分の入力で破らせないため、入口で弾くようにした。実行して確認済み（`Fatal error` が `jst-date.ts:124` → `generate-articles.ts:1313` で、Bedrock / Tavily 到達前）。

空文字列・空白のみは従来どおり「未指定」扱い（schedule 実行で `inputs.publish_date` が空になる経路を壊さないため）。

### 6. ⚠️ ミュータント検証で 1 種が生存し、テストの弱点を指した（次回以降の再利用価値あり）

8 種のうち **④「形式チェックを外す」が初回は生存した**（37 テスト全通過）。原因は、形式チェックを外しても**実在チェックが同じ入力を捕まえて例外を投げる**ため、`toThrow(/PUBLISH_DATE/)` では 2 つの分岐を区別できなかったこと。

エラー理由まで検証する形（`/PUBLISH_DATE は YYYY-MM-DD 形式/` と `/存在しない日付/` を別々に照合）に直したところ④も 7 件失敗で検出されるようになった。最終的な検出数:
...
| ⑤実在チェック除去 | 6 |
| ⑥明示指定を無視 | 16 |
| ⑦Invalid Date ガード除去 | 1 |
| ⑧`\d{4}` → `\d+` | 2 |

**教訓: 「例外が飛ぶこと」だけを検証すると、複数の検査が同じ入力を捕まえる場合に片方を外しても気づけない。** 検査を 2 つ並べたら、どちらの理由で落ちたかまで検証する。

### 7. 分離した Issue 2 件（どちらも `monitoring`）

| Issue | 内容 |
|---|---|
| **#323** | **選定（`fetch-data`）が `PUBLISH_DATE` を尊重しない。** PR-C の申し送りは「`buildIndieCandidates` が `publishDate` を使っていない件は #308 と同じレイヤー」としていたが、**実際には層が違った**。選定側は PR-C で既に JST 基準（`getJstDateString` / `getJstDayStartUnixSec`）になっており、残っていたのは「`fetch-data` は別プロセスで `PUBLISH_DATE` を渡されていない」＝**過去日を指定した再生成が当時の選定を再現しない**という別問題。運用意図（当時を再現したいのか今のデータで作り直したいのか）を先に決める必要がある |
| **#324** | **`isEventOnDate` がローカル時刻のゲッター（`getDate()` 等）で UTC 0 時の Date を読んでいる。** 負オフセットの TZ では 1 日ずれる（実測: `TZ=America/Los_Angeles` で `new Date('2026-08-15').getDate()` = **14**、UTC / JST では 15）。CI=UTC・開発機=JST でのみ動かしているため**現に壊れてはいない**。**#310 が同ファイルに後方探索を足すので、そのとき一緒に直すのが最も安い** |

### 8. 対応しなかったこと（記録）

- **公開済み 19 号の `publishDate` は遡って直していない。** 本PRは新しく生成される号にのみ効く。金曜のままの 13 号を直すには記事の再生成が必要になるため意図的にスコープ外にした
- **`/code-review` の指摘は 0 件**（5 エージェント中 3 つが指摘なし）。出た 2 件はいずれも採点 0 で不採用: ①コミットメッセージの日付と明示指定 `publish_date` の不一致（**修正前から同じ**挙動で本PRが悪化させていない） ②`Build summary` の `Today` 表記（**本PRが触っていない行**）
- **`fetch-japanese-events.ts` の CLI デバッグブロック**（`today.toISOString().split('T')[0]`）は UTC 基準のまま。デバッグ出力専用で号の発行日には影響しない

### 9. 教訓

**申し送りの「層の判定」も検証対象。** PR-C の申し送りは `buildIndieCandidates` の件を「#308 と同じレイヤー」と書いていたが、実際に読むと選定側は既に JST 化されており、残っていたのは別軸（`PUBLISH_DATE` の伝播）だった。**申し送りは「見るべき場所」としては正しく、「同じ問題」という判定は誤り**という形の不正確さがある。前セッションの結論も一次ソースで確認する。
...
## 実施結果（2026-08-13。PR #326。マージ `6bd8041`。→ Issue #310 / #324 は Closed）

**31ファイル / 1432テスト全通過**（着手前 30 / 1364。新規 1 ファイル・+68 テスト）。コミット 2 本
（実装 `ec95674` → `/code-review` 指摘対応 `20abe2e`）。

### 着手前検証: Issue の主張は再現したが、実測表の定義が 1 点ずれていた

`#308` マージ後の main で測り直した（申し送りの指示どおり）。**中心的な主張はすべて再現**した:

| 検証項目 | 結果 |
|---|---|
| イベント 0 件週（土曜基準） | **5 週**（04-11 / 05-23 / 06-13 / 08-22 / 09-12）。期待値どおり |
| 過去方向の採用候補 | 5 週すべて **-1〜-5 日**で埋まる。仕様書 §4.3 の表と**完全一致**（駅弁の日 / 国際博物館の日 / 恋人の日 / 俳句の日 / 救急の日） |
| `getEventsInRange` は過去を見ないか | 実読で確認（`for (let i = 0; i < days; i++)`） |
| 固定文言の場所 | **doc の行番号が両方ずれていた**（`:616` / `:1043` → 実際は `:743` / `:1173`）。修正対象は前者のみ、後者は呼び出し元ゼロのまま |
| 実データの前提 | version 1.2 / 127 件（テストで固定した） |

⚠️ **食い違い 1 件（実装は止めなかった）**: Issue 本文・仕様書 §4.3 の「未来方向で最も近い記念日」の列が
2 行おかしい。**2026-04-11 は「11 日後 アースデイ」ではなく +7 日 発明の日**（4/18）、
**2026-08-22 は「9 日後 野菜の日」ではなく +7 日 焼肉の日**（8/29）。
元の測定が **+8 日以降**を探していたためで、記念日データ自体は実在する（`grep` で確認）。
**「過去方向のほうが近い」という結論は変わらない**（-1 日 vs +7 日 / -3 日 vs +7 日）ので実装は継続し、
仕様書側に訂正ブロックを追記した。この再測定から「§4.4 の『8 日以降』は**オフセット +7 以降**（窓の直後から連続）」
という実装判断が出た——ここを +8 から始めると 04-11 / 08-22 で 1 日分の空白ができる。

### 決めた下位判断（§9.2-8 / §9.2-9）

| 判断 | 決定 | 根拠 |
|---|---|---|
| 記念日の記録先 | `history.json` の新配列 `featureEvents` | 書き込み経路が 1 つで済む（`saveHistory` の第 2 引数）。**リポジトリにコミットされるので唯一の永続記録**になる。記念日名は `japanese-events.json` の `name` そのままなので、ゲームタイトルのような表記ゆれ問題（§6.3 の既知の限界）が構造的に起きない |
| 除外する号数 N | **2 号**（`FEATURE_EVENT_EXCLUDE_ISSUE_COUNT`。`0` で無効化） | 1 号で構造的な重なりは防げる（**週次発行なら過去方向 7 日の窓は前号の未来方向 7 日の窓と完全に一致する**）。2 号は発行日が数日ずれた場合の保険で、2 号前の窓は過去方向の探索範囲と重ならないためコストがゼロ |
| フォールバックの可視化 | 生成時 WARN + `build-issue` の CI 出力 + `history.json` の `source` | §9.2-9 の要求は「ログ**または**記事データ」。**Validation Report には載せなかった**（下記） |
| 除外を効かせる段階 | **3 段階すべて** | 段階 1（未来方向）での除外は週次発行なら実質何もしない（隣接号の窓は重ならない）が、発行日がずれたときの保険になる。**過去方向の窓は前号の窓と完全に重なるため、除外は段階 2 で本質的に必要** |
| 過去方向の採用範囲 | **最も近い 1 日分のみ**（同日に複数あれば全件） | §4.4 が「最も近いものを採用」と定めている。未来方向は従来どおり窓内の全件を LLM に渡す（既存挙動を変えないため）。**この非対称は意図的** |

### `/code-review` の結果と採否

5 エージェント中 3 つが指摘なし。出た 2 件はいずれも**採点 80 未満**（50 / 75）でしきい値未達のため
PR へのコメントは投稿されていない。管理者判断:

1. **採用（採点 50）** — `matchSelectedEventName` の部分一致が `find` で先頭一致を拾っており、
   包含関係のある記念日名で短い方に誤同定し得た。**全 365 日の窓で実測すると 2026 年では発火 0 件**
   （短い側が先に並ぶ窓が存在しない）だが、3 行で塞げるので最長一致に修正し回帰テスト 2 件を追加。
   **「潜在的」と「無害」は違う**——`japanese-events.json` の改訂で成立する
2. **不採用（採点 75）** — 「#271 / #303 の前例に倣い Validation Report にも載せるべき」。
   §9.2-9 の要求は「ログまたは記事データ」で 3 系統満たしており、**レポートは検出した「問題」を載せる場**である。
   #303 が low 警告を足したのは**安全性検査のスキップ（実際の穴）**で、設計どおりの動作である本フォールバックとは
   性質が違う。永続記録が必要という論点は `history.json`（コミットされる）で満たしている

### Issue #324（TZ 依存）を同時に解消した

解除条件①「#310 に着手するとき」を満たしたので同 PR で修正した。**同ファイルの同じ関数群を触るので実際に安かった。**

| | JST / UTC | America/Los_Angeles（修正前） |
|---|---|---|
| 2026 年のイベント 0 件週 | 5 週 | **3 週**（05-23 / 08-22 / 09-12） |
| 2026-08-22 の過去方向 | -3 日 俳句の日 | **-2 日** 俳句の日 |
| 2026-05-23 の過去方向 | -5 日 国際博物館の日 | **-4 日** 国際博物館の日 |

- `isEventOnDate` のローカル時刻ゲッター → `getJstDateString` 経由の `getJstDateParts` に置換
- `getNthWeekdayOfMonth` は**ローカル 0 時 → UTC 0 時**の Date を返すよう変更（`Date.UTC` / `getUTCDay` で計算）
- 日付加減算を date-fns（`addDays` / `addWeeks` / `setDay` / `startOfMonth`。**すべてローカル時刻ベース**）から
  UTC のミリ秒演算に変更。**このファイルから date-fns 依存が消えた**
- 5 TZ（JST / UTC / LA / ホノルル / キリバス +14）で同一結果になることをテストで固定し、
  **`TZ=America/Los_Angeles npm run test` で全 1432 テスト通過**を確認

### ミュータント検証 12 種すべて検出（うち 1 種は初回に生存）

①過去方向の上限（`<=` → `<`）②除外フィルタの無効化 ③未来方向を 1 日分に縮める
④日付解釈をローカル時刻に戻す ⑤拡張未来方向の開始を +8 にする ⑥除外号数の `<` → `<=`
⑦記念日だけの保存で早期 return ⑧過去方向で全日分を返す ⑨**完全一致判定を殺す**（初回生存）
⑩`featureEvent` を記事データに載せない ⑪build-issue が特集以外も記録する
⑫部分一致を先頭一致（`find`）に戻す。

⚠️ **実装コミット `ec95674` のメッセージは「12 種」と書いているが、その時点で走らせていたのは ①〜⑪ の 11 種**
（⑫は `/code-review` 対応コミット `20abe2e` で追加した）。**最終状態が 12 種**で、PR 本文の記述はこちらに一致する。

⚠️ **初回に生存したのは「完全一致判定を殺す」ミュータント。** 部分一致（`includes`）が同じ入力を拾うため、
完全一致を消してもどのテストも落ちなかった。**#308 の「2 つの検査が同じ入力を捕まえるため `toThrow` だけでは
区別できない」と同型の弱点**で、包含関係のある候補名（実データに 5 組: `肉の日 ⊂ 焼肉の日` /
`猫の日 ⊂ 世界猫の日` / `猫の日 ⊂ 招き猫の日` / `クリスマス ⊂ クリスマスツリーの日` /
`クリスマス ⊂ クリスマスイブ`）を使うテストを足して初めて検出できた。

### 受け入れ条件の達成状況

| 条件 | 結果 |
|---|---|
| 5 つの 0 件週で過去方向の記念日が採用されることを実データで確認 | ✅ 5 週すべてテストで固定（実 `japanese-events.json` を読む） |
| 境界値テスト（過去 7 日ちょうど / 8 日前） | ✅ 合成データで両方（+ 窓の +6 / +7、拡張未来方向の開始位置も） |
| 通常週の挙動が不変であることをテストで固定 | ✅ 2026 年の全 52 土曜で `source` と候補配列を従来の窓と突き合わせ（47 週が `forward` で一致） |

### やらなかったこと

- **DEV_MODE の号生成（`build-issue:dev`）は実行していない。** テーマ選定経路は 0 件週の実データで
  単体・結線テストまで通しており、Bedrock コストを払って得られる追加の確度が薄いと判断した。
  PR-E（#307）は受け入れ条件が DEV_MODE 検証を明示していたので実行したが、#310 の受け入れ条件は
  「実データで確認する」までで、テストで満たしている
- **`determineFeatureTheme`（`bedrock-client.ts:1187`）は削除していない。** 呼び出し元ゼロの死んだコードで
  固定文言の 2 箇所目を持つが、PR-F 節が当初から「削除するなら別PR」としているためスコープ外に置いた
- **`saveHistory` の既存エントリ側には同一呼び出し内の重複除去を足していない**（`featureEvents` 側には足した）。
  レビューで非対称性を指摘されたが、既存挙動の変更になるため見送った（実害の観測もない）
...
**PR #328。マージコミット `6567ed1`（通常マージ、squashではない）。コミット1本（`6e13dc7`）。**
**`/code-review` は 5 エージェントすべて指摘ゼロ**だったため追加コミットは無い。

**§8 実装計画表の最後の 1 行。これで 15/15 完了。**

### 着手前検証で判明した 2 点（Issue 本文の更新）

**1. 実測表が陳腐化していた。** Issue 本文・仕様 §6.4 の「期待 6 本を満たしたのは 17 号中 14 号」は、
着手時点では **19 号中 14 号**（不足は 5 号）。`src/content/issues/issue-*.md` の frontmatter を
全件数えて確認した:

| 号 | 新作 | インディー | 特集 | 名作 | 計 | 本番レポートの status |
|---|---|---|---|---|---|---|
| 001〜012, 014, 016 | 2 | 2 | 1 | 1 | 6 | （017 より前はレポート未保存） |
| **013** | 2 | 2 | 1 | **0** | **5** | （レポート未保存） |
| **015** | **1** | 2 | 1 | 1 | **5** | （レポート未保存） |
| **017** | **0** | 2 | 1 | 1 | **4** | `error`（high=3 が理由） |
| **018** | **1** | 2 | 1 | 1 | **5** | `error`（high=1 が理由） |
| **019** | **0** | 2 | 1 | 1 | **4** | **`warning` → Issue 未起票** |

**直近 5 号のうち 4 号が不足**（015 / 017 / 018 / 019）。Issue が「実測で該当は 17 号中 3 号なので
毎週鳴るノイズにはならない」と書いていた根拠は、**19 号時点では 5/19 = 26% に増えている**。

**2. 予測された実害は仮説ではなく既に発生していた。** vol.019（**Issue 起票の 3 日前に発行**）は
新作 0 本・記事 4 本だが、`data/validation/validation-report-019.json` は high=0 / medium=1 で
**`status=warning`**。したがって Issue は自動起票されておらず、Issue 本文が予測した
「high が 0 件なら新作 0 本でも気づけない」状態が**本番で実現していた**。
**この事実は Issue 本文にも仕様書にも書かれておらず、`data/validation/` を読むだけで見つかった。**

### severity: 仕様 §6.5 の「high」を採らなかった（ユーザー確認済み）

仕様 §6.5 は「重大度 high」と定めていたが、**high 警告として計上せず、独立した判定項**
（`ValidationReport.articleCountShortfalls`）にした。ユーザーに 3 案（high 警告として計上 /
独立判定項 / medium にとどめる）を提示して決定した。理由 2 点:

1. **§9.1 保留1 との切り離し。** high 警告は記事**本文**の品質（ハルシネーション・整合性）の指標で、
   その重大性の再定義が未決着（保留1）。**号の構成**の問題である本数不足を同じバケットに混ぜると、
   保留1 の議論対象がさらに不均一になる。独立項なら保留1 の結論に依存しない
2. **`warningsBySeverity.high` は他の判断にも使われる数値。** `writeAndCheckReport` の fail 閾値
   （既定 5 件）と自動再生成の対象判定に効くため、本文品質以外の要因で動かさない

前例は **#222 の `adultScreeningFailures`**（専用フィールドを持たせて `computeReportStatus` の
別項として error に昇格）。
...
**期待本数は定数 + 環境変数の上書き。** `EXPECTED_ARTICLE_COUNT_NEWRELEASE` / `_INDIE` /
`_FEATURE` / `_CLASSIC`（既定値 2 / 2 / 1 / 1）。前例は #310 の
`FEATURE_EVENT_EXCLUDE_ISSUE_COUNT` で、**`0` を「そのカテゴリの検出を無効化する」有効な指定**
として扱うため `Number(raw) || default` とは書いていない（0 が既定値に化ける）。
負数・非整数・空文字は不正な指定として既定値に落とす。

### スコープに足した判断: hidden 記事を本数に数えない（ユーザー確認済み）

`hidden: true` が付く記事（`criticallyIncomplete` = Issue #94 / `game-source-mismatch` = Issue #179）は
読者に表示されないため、掲載本数から除外する。`build-issue.ts` が既にクールダウン対象から
hidden を除外しているのと同じ扱いに揃えた（`collectHiddenArticleTitles` を新設し、
`formatArticleForFrontmatter` が `hidden: true` を書く 2 条件と一致させている）。

**実測で該当は 1 件だけ**: vol.001 の indie 記事 1 本（`grep -c "hidden: true"` で過去 19 号中
**この 1 件のみ**）。この判断により vol.001 が新たに不足（indie 1/2）として検出される。

`validate-existing-issue.ts`（公開済み号の手動監査ツール）にも frontmatter の `hidden` を
読ませて同じ扱いにした。

### ワークフローの Issue タイトル（#222 と同じ対応）

本数不足のみで起票された号は high=0 なので、従来のタイトル組み立てでは
**「HIGH: 0件」という矛盾表記**になる（#222 の `/code-review` が指摘したのと同型）。
`DETAIL` を条件付きで後ろに足す形に変え、**`ADULT` と `SHORTFALL` がどちらも 0 のときは
従来と完全に同一の文字列**になるようにした（重複防止の完全一致フィルタが旧 Issue と
一致し続ける必要がある）。shell + jq で 3 パターンを実測して確認した。

### サイトの「毎号6本」記述（5 箇所）

`src/pages/launch.astro` の 5 箇所を「最大6本」に修正した。**Issue 本文が挙げた行番号
...
判別できない不整合が生じた。表現の方針決定が先なので Issue に分離した → ✅ **解消**（2026-08-14。PR #334。マージ `21b6d6e`）。カード 4 箇所を「毎号 最大 N 本掲載」にし、CONTENTS の説明文に編集方針を明記した。**着手時に、同じカード内の `content-card-count` 4 箇所を見落としていたことが判明**（合計 8 箇所）。後者は `opacity: 0.08` の装飾ウォーターマークなので対象外とした。

### 検証

- **31 ファイル / 1469 テスト 全通過**（着手前 31 / 1432。新規 37 件）
- **実データ**: 公開済み 19 号すべてで判定を再現。**status が変わるのは vol.019 の
  `warning` → `error` だけ**（017 / 018 は既に high 由来で error）。期待どおり 6 本ある
  14 号は不足 0 件で**誤検出なし**
- **ミュータント検証 9 種すべて検出**（判定項の削除 / 不足判定を `<=` に緩める / `expected - 1` に
  狭める / hidden 除外の削除 / 環境変数の 0 が既定値に化ける / 不足 0 件を `undefined` にする /
  カテゴリ数と不足本数合計の混同 / hidden 集合の `||` → `&&` / 期待本数の既定値の改変）。
  **2 セッション続いた「1 種生存」が止まった。** 効いたのは境界値を
  **「同数 / 1 本手前 / 超過」の 3 点**で書いたことで、`<=` 方向と `expected - 1` 方向の
  両ミュータントが別々のテストで落ちた。**不等号の判定は片側だけのテストでは両方向を区別できない**

### 副産物: エントリポイントガードを持たないスクリプトが 3 本ある（未起票）

一時スクリプトから `build-issue.ts` を import したところ、**`main()` が実行され始めた**
（`Next issue number: 20` まで進行）。原因は末尾の `main().catch(...)` が無条件呼び出しで、
`generate-articles.ts` / `validate-article.ts` が持つ
``if (import.meta.url === `file://${process.argv[1]}`)`` ガードが無いこと。実測:

| スクリプト | ガード |
|---|---|
| `build-issue.ts` | ❌ 無し |
| `fetch-data.ts` | ❌ 無し |
| `validate-existing-issue.ts` | ❌ 無し |
| `generate-articles.ts` / `validate-article.ts` | ✅ あり |

**ガードの存在は既に前提として使われていた**: PR-0 の「実施結果」節は `generate-articles.test.ts` を
...
`npm run test` のたびに本番パイプラインが起動する。→ **Issue #330**（#311 とは別層のため PR #328 には含めず分離）→ ✅ **解消**（2026-08-14。PR #333。マージ `0817339`）。
...
| ファイル | 行数 | ずれ |
|---|---|---|
| `scripts/validate-article.ts` | 1264 → **1379** | 旧 25〜47 は +1 / 旧 54〜92 は +15 / **旧 99〜1045 は +26** / 旧 1054 以降は段階的に増え、旧 1192 以降は **+115** |
| `scripts/format-validation-report.ts` | 286 → **357** | 合計 +71。関数の頭で見ると `computeReportStatus` 旧 59 → **88**、`shouldFileIssue` 旧 79 → **114** |
| `scripts/build-issue.ts` | 770 → **796** | 旧 23〜138 は +1 / **旧 157〜167 は −5**（`categoryToJapanese` の本体が 11 行 → 5 行に縮んだ） / **旧 174〜713 は +16** / 旧 721 以降は **+26** |
| `scripts/validate-existing-issue.ts` | 167 → **180** | 旧 21 以降 +2 / 旧 155 以降 +13 |
| `.github/workflows/weekly-build.yml` | 199 → **205** | 旧 137 以降 **+6**（`File issue on validation error` ステップの見出しは **118 行目のまま**） |
| `src/pages/launch.astro` | 1398 → **1398** | **ずれなし**（5 箇所すべて同一行での置換） |

### 行番号のずれ（#330 / #331 の 2 PR による。PR-G の表に続く分）

`abe0113`（#332 マージ時点）から `21b6d6e`（#334 マージ時点）までの差分。

| ファイル | 行数 | ずれ |
|---|---|---|
| `scripts/build-issue.ts` | 796 → **799** | 旧 25〜788 は **+1** / 旧 789 以降は **+3** |
| `scripts/fetch-data.ts` | 1539 → **1542** | 旧 31〜1531 は **+1** / 旧 1532 以降は **+3** |
| `scripts/validate-article.ts` | 1379 → **1380** | 旧 19 以降は一律 **+1** |
| `scripts/generate-articles.ts` | 1637 → **1638** | 旧 47 以降は一律 **+1** |
| `scripts/validate-existing-issue.ts` | 180 → **184** | 旧 14〜176 は **+1** / 旧 177 以降は **+4** |
| `src/pages/launch.astro` | 1398 → **1398** | **ずれなし**（#331 も同一行での置換） |

いずれも `import { isMainModule }` の 1 行追加と、末尾の `main()` 呼び出しを `if` で包んだ分だけ。
**PR-G の表に載せた大きなずれ（`validate-article.ts` の +115 等）とは別物なので、両方を足して計算しないこと。**

**#330 / #331 で新しく参照先になった位置**（マージ後の main で直接 grep した値）:

| 対象 | 現在の位置 |
|---|---|
| `isMainModule`（判定本体） | `scripts/entrypoint.ts:40`。両方向の候補比較は `:57`、realpath 解決は `:74` |
| ガードの適用箇所（5 本） | `build-issue.ts:794` / `fetch-data.ts:1537` / `generate-articles.ts:1633` / `validate-article.ts:1375` / `validate-existing-issue.ts:182` |
| カテゴリ別カードの本数表記（4 箇所） | `launch.astro:1216` / `:1228` / `:1240` / `:1251` |
| CONTENTS の説明文（編集方針を明記した箇所） | `launch.astro:1202` |

**主要な参照先の現在位置**（オフセット計算ではなく、マージ後の main で直接 grep して確認した値）:

| 対象 | 現在の位置 |
|---|---|
| `computeReportStatus`（Issue #311 が旧 `:57-70` として引用） | `format-validation-report.ts:88-106` |
| `shouldFileIssue`（同 旧 `:80`） | `format-validation-report.ts:114` |
| `game-source-unchecked` 警告の生成（#296） | `validate-article.ts:736-748` |
| `game-source-unchecked` の CI 出力（#296） | `build-issue.ts:593-597` |
| `game-source-unchecked` のレポート記録（#296） | `build-issue.ts:764-768` |
| `VALIDATION_STRICT` の判定（**リポジトリ内で唯一の出現箇所**） | `build-issue.ts:784` |
| Issue 自動起票ステップ（PR-G 節が旧 `:113-160` として引用） | `weekly-build.yml:118`（見出しは不変。本文が +6 行） |

> ⚠️ **#296 が引用していた `build-issue.ts:532-540` / `:682-687` に本 PR のオフセットを足しても
> 現在位置にはならない。** それらは PR #303 時点の行番号で、間に別 PR が入って既にずれていた。
> **「前回の doc の行番号 + 今回のずれ」で計算するのではなく、毎回 grep で直接引き直すこと。**
...
# #26 対応: 早期アクセス（Early Access）状態の取得と表記の是正

**PR #340（マージ `f544886`。2026-08-14）/ Issue #26（Closed）/ 分離: #339**

## 実施結果

### 着手前検証で分かったこと（Issue の 3 つの「原因（推測）」の当たり外れ）

Issue #26 は 2026-05-29 起票で、**一度も更新されていなかった**（コメント 0 件）。本文の「原因（推測）」3 項目を独立に検証した結果:

| Issue の原因推測 | 検証結果 |
|---|---|
| ① `releaseDate` が二値判定で**早期アクセスの概念が無い** | ✅ **真**。`earlyAccess` / `early_access` / `早期アクセス` の実装ヒットは **0 件**。仕様書・決着ブロックにも記述なし（= 「決着済みだが未実装」ではなく**仕様未定義**） |
| ② IGDB `first_release_date` が**早期アクセス開始日とズレる（1 日）** | ❌ **一次ソースで再現しない**。`Slay the Spire II` は IGDB `2026-03-05` / Steam JP ストア「2026年3月5日」で一致。`サブノーティカ２` も IGDB `2026-05-14` / Steam JP「2026年5月14日」で一致。judge が矛盾根拠にした「3月6日」「5月15日」は **Tavily の検索結果（記事・動画タイトル）由来**だった |
| ③ **プラットフォームごとに発売日が異なる**場合を表現できない | ✅ **真だが件数は小さい**。80 記事中 9 件（確定日が後発なのは 2 件）→ **#339 に分離** |

### 実害の実測（公開済み 19 号 / 107 記事）

**Steam ストアが早期アクセスと表示しているタイトルは 8 タイトル / 10 記事。**

⚠️ **IGDB `game_status=4` だけで数えると 14 記事になるが、Steam で裏取りすると 4 件落ちる。**

| 落ちた理由 | 件数 | 実例 |
|---|---|---|
| Steam では既に早期アクセスを終えていた（IGDB の偽陽性） | 1 | `Realm of Ink` |
| Steam 非配信（判定材料が無い） | 2 | `Hytale` / `Dungeon Blitz R` |
| Steam 検索が別作品にヒットして判定不能 | 1 | `Star Citizen`（Steam 未配信。検索は『シチズンスリーパー2』を返した） |

確認できた 10 記事の内訳:

| 観測 | 件数 | 実例 |
|---|---|---|
| 本文で「発売中」等と断定 | **5** | vol.007『サブノーティカ２』/ vol.007『R.E.P.O.』/ vol.008『ARK: Survival Ascended』/ vol.011『Witchspire』/ vol.012『Slay the Spire 2』 |
| そのうち早期アクセスに**一言も触れていない** | **1** | vol.008『ARK: Survival Ascended』 |
| 断定していないが早期アクセス無言及（参考） | 3 | vol.001『Dig Island』/ vol.008『ARK…』/ vol.018『SAND: Raiders of Sophie』 |

⚠️ **PR #340 の本文・コミットメッセージ・仕様 §2.9 の初版は「11 記事」と書いていたが誤りだった。** 本 docs PR の作成時に `git show` ではなく実測データを数え直して発覚した。**マージ済みのコミットメッセージは訂正できないため、本節と §2.9 が正である。**

judge 側の実績（手元にある検証レポート **25 本**（本番 3 / dev 22）のうち **12 本**に、発売日・早期アクセス関連の指摘が計 **18 件**）:

> 📏 **この 18 件の測り方**（件数だけ書くと再現できないので明記する）: `data/validation*/validation-report-*.json` の
> `llmJudge.warnings[].message` に **`早期アクセス` / `Early Access` / `正式リリース` / `発売日` / `発売され` / `発売中` の
> 6 語のいずれか**が含まれるものを数えた。**語を足すと件数は動く**（例: `リリース` を 7 語目に加えると 19 件、
> `配信` を加えると 20 件 / 13 本になる）。`warnings`（正規表現バリデータ側）を含めても 18 件で変わらない。


| レポート | 記事 | judge の指摘（確信度） |
|---|---|---|
| 023 | サブノーティカ２ | 「2026年5月14日に配信が開始された」は矛盾。検索結果は「slated to release」（90%） |
| 011 / 016 | Slay the Spire II | 「発売日は2026年3月5日」は矛盾。検索結果は「3月6日に早期アクセスの配信を開始」（95%） |
| 012 | Timberborn（特集） | 「2026年3月13日に発売された」は矛盾。検索結果に「アーリーアクセス版」（90%） |

**ただし ② のとおり、この judge の指摘は一次ソースでは記事の側が正しかった。** judge の確信度は「検索結果に対する確信度」であって「事実に対する確信度」ではない。

### 実装の設計判断

| 判断 | 内容と理由 |
|---|---|
| 発売状態（§2.8 の 3 値）に**混ぜない** | 早期アクセスは「発売済み（購入してプレイできる）」と「正式リリース前」を同時に満たす直交軸。`ReleaseStatus` に 4 値目を足すと、発売状態を参照する 5 箇所（本文プロンプト・見出しプロンプト・Web 検索セット・評価断定バリデータ・見出しの未発売表現バリデータ）すべての解釈表を書き換えることになる |
| 本文表記の一次ソースは **Steam の `genres` id 70 のみ** | IGDB `game_status` は実測で両方向に外れる（`Realm of Ink` = 偽陽性 / `ARK: Survival Ascended` = 偽陰性。**ライブデータでも `Hot Dogs, Horseshoes & Hand Grenades` で偽陽性が再現**）。本文で偽陽性を踏むと**正式リリース済みの作品に新たな誤情報を足す** |
| 名作枠のゲートだけ **IGDB も併用** | ゲートの偽陽性は「候補が 1 件減る」だけ。母集団は実測 279 件あるので実害が小さい。偽陰性（早期アクセス作品を名作として深掘り）は §5.1 の約束と正面から矛盾する |
| 「発売中」自体は**禁止しない** | Steam ストアでも購入可能な状態として案内されており事実に反しない。実測した事故は「早期アクセスに触れずに完成品として紹介していた」点が問題だった |
| バリデータは **`warning` 止まり** | 一次対策はプロンプト側。この判定項は発火頻度が未観測の観測網なので、`error` にすると `shouldFileIssue` が真になって号ごとに Issue が自動起票される（#222 の `unrecognizedScreeningResponses` と同じ判断） |
| 追加 API コストは **ゼロ** | `appdetails` は全 Steam 候補に対して既に呼んでおり、`genres` は同じレスポンスに含まれていた |

### 🚨 IGDB の `!=` は null のレコードも落とす（`games/count` で実測）

名作枠の母集団条件（`total_rating >= 85 & total_rating_count >= 200 & game_type = (0,8,9) & themes != (42)`）に追加した場合:
...
| `& (game_status = null \| game_status != 4)` | **278** ← 採用した形 |

名作級のタイトルは `game_status` が null であるのが普通（`Grand Theft Auto V` も `The Witcher 3` も null）。**除外条件を足すときは件数を測ること。**

供給への影響は **279 件中 1 件**（`Path of Exile 2`。`total_rating=88.4` / `total_rating_count=205`）。現在は `limit 200` の窓の外だが、評価母数が伸びれば入ってくる。

### ミュータント検証 23 種（1 種が初回に生存）

⚠️ **`needsCompletion` に足した「早期アクセス未判定」条件を外してもテストが落ちなかった。** 原因は本物で、**その条件は現状では単独で判定を変えない**——`steamRecommendations` を書くのが同じループだけなので、1 周目は必ず未設定になり他の条件が先に true になる。#280 の「第 2 引数が到達不能だった」と同型。

対処: 純関数 `needsStorefrontCompletion`（`fetch-data.ts:196`）に切り出し、「全フィールドが埋まっていて早期アクセスだけ未判定」の入力を単体テストで与えて意図を固定した。これでミュータントが死んだ。**「防御的に足した条件」の死活はミュータント検証で分かる。**

### DEV_MODE での実データ検証

**今週の選定 5 本のうち 2 本が早期アクセスだった。** 修正が次の定期実行で即座に効く状態だった。

| 記事 | `isEarlyAccess` | 生成結果 |
|---|---|---|
| `Sandustry サンダストリー`（新作） | `true` | 見出し「…『サンダストリー』早期アクセス配信中」/ 発売情報「**早期アクセス配信中**」/ 要約にも記載 |
| `Backrooms: Escape Together`（インディー） | `true` | 見出しに「早期アクセス配信中の」/ 発売情報「発売日: 早期アクセス配信中（2022年10月18日より）」+ 注記 |
| `Big Walk`（インディー） | `false` | 「発売日: 発売中」。早期アクセスに触れない（誤記なし） |
| `Splatoon Raiders`（新作） | `undefined`（未判定） | 早期アクセスに触れない（**未判定では何も書かせない設計の実データ確認**） |
| `The Witcher 3: Wild Hunt`（名作） | `false` | 影響なし |

- 判定カバレッジ: `steamAppId` を持つ **218 件中 203 件が判定済み**（93%）。未判定 15 件は Storefront の取得失敗によるもの
- レポート: `Early access statement issues: 0`（プロンプトが効いたので発火しない）。`status=warning` は既存要因（medium 2 件・公式URL未取得）由来で、EA 判定項は押し上げていない
- 公開 Markdown の frontmatter に `isEarlyAccess` は漏れていない（テストでも固定）

### `/code-review`

**5 エージェントすべて指摘ゼロ**（採点対象 0 件のため PR コメントは投稿されていない）。過去に指摘されたパターンは着手時に先に潰してあった: #299/#305 の `deduplicateGames` マージ漏れ（`??` と `||` の使い分けを含む）/ #271 の「未計測（undefined）と計測して 0 件」の区別 / #333 の死んだコード / #320 のプロンプト内の矛盾指示。

### 判定項の再開条件

`earlyAccessStatementIssues` が **2 号以上連続で発火**した場合は、プロンプトの効き方を見直し `error` 昇格の要否を検討する。

### 次に触るときの注意

- **早期アクセス以外の `game_status` は未決**（Alpha=2 / Beta=3 / Offline=5 / Delisted=8）。名作枠の母集団に `Offline`（`Overwatch`）と `Delisted`（`Forza Horizon 4`）が計 5 件あることは実測済み。「サービス終了作品を名作として深掘りしてよいか」は本 Issue のスコープ外
- **`isEarlyAccess` は `steamAppId` を持つゲームしか判定できない**。`reconcileSelectedGames` は選定後に `steamAppId` を解決することがあるため、そこで初めて appId が判明したゲームは未判定のまま記事化される（実測: `Splatoon Raiders`）。実害は「早期アクセスに触れない」だけなので追加実装は入れていない
