...
- **事後の同一性照合ではこのクラスを検出できない。** 混入した値が照合の裏付け軸（year / company）を自分で供給するため `uncertain` にしかならない。**防御はメタデータ採用時点にしか置けない** → 未防御経路を **#317**、それに使える孤立関数を **#318** として分離（どちらも実測0件で `monitoring`）
