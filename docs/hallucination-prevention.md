...
特集記事（`category: feature`）は、`enrichRecommendedGames()` で取得した `recommendedGames` の metadata（`platforms` / `developer` / `publisher`）をもとに以下を実施：
- `platform-mismatch`: 全推薦ゲームのプラットフォームの合算を許容セットとして検証
- `person-*`: 全推薦ゲームの `developer` / `publisher` を許容リストとして人物言及を検証
- `numeric-*`: 数値クレームを検出（`sourcedFrom` は Tavily 未使用のため常に `undefined`）

`recommendedGames` にプラットフォームデータが存在しない場合（IGDB で取得できなかった場合）は platform-mismatch チェックをスキップ。
...
`sourcedFrom` は newRelease・indie・classic 記事にのみ付与される（feature 記事は Tavily 未使用のため常に `undefined`）。  
feature 記事の platform-mismatch / person-* は `recommendedGames` の IGDB metadata に依存するため、IGDB で取得できたゲームのみが有効な許容セットになる。
...
| 特集記事内の vehicle-count（例: 「550台以上の実車」） | Tavily 未使用のため `sourcedFrom` が常に `undefined` | 文脈で判断（low/medium 扱いで人間確認） |
...
**特集記事のゲームエンティティ対応**（実装済み）
- `enrichRecommendedGames()` が IGDB から `platforms` / `developer` / `publisher` を取得
