...
- **万・億・千を含む表記は下位桁まで1マッチに束ねる**（Issue #391）。`3万5000人` を `3万` と `5000人` に分割せず `3万5000人` として1つの警告にする。万・億・千の連鎖（`1億2000万5000人` / `1億2千万人`）も1マッチに。千のみ（`5千人` / `5千円`）も検出対象。理由:
  - evidence が本文に存在しない断片（`5000人`）になると、棚卸しで本文を検索しても見つからない
  - `buildFixInstruction` が「数値『5000人』は提供データにありません。削除してください」という、本文に無い数値の削除指示を生成する
  - 本来検証したい「3万5000人」そのものが提供データと突き合わされず、high 件数の判定も汚れる
- 平地の数値パターン（カンマ区切り・4桁以上）には否定後読み `(?<![\d,万億]\s*)` を付与し、万・億表記の下位桁を断片として拾わない。**空白を挟む表記（`2万 5000人`）でも断片化しない**
- 価格（`numeric-price`）は3桁以下・小数も検出対象（`19.99ドル` / `60ドル` / `980円`）。ドル価格は2桁が普通なので、4桁以上に限定すると実質未検出になる
...
- **漢数字の一般形**（`五千人` `二万人`）。`\d+千`（例: `2万5千人` / `5千人`）だけ対応する
- **台・種・時間・%・周年 の万表記**（`1万2000台` 等）。非現実的な数値のため万・億の変種は追加しない。否定後読みを追加した結果、断片ではなく**未検出**になる（断片の evidence より未検出のほうがまし、という判断）
...
#### `game-type-*` の設計と未検出パターン（Issue #387）

**未言及方向を newRelease に限定した根拠:** `gameType` を執筆プロンプトに渡しているのは新作枠だけ（`generate-articles.ts` の `buildUserMessage('newRelease', …)` に `gameType` があり、`'indie'` / `'classic'` には無い）。渡していない情報の欠落を警告すると、執筆AIには直せない指摘になる。indie / classic に広げるには執筆プロンプト側に `種別` を渡す変更（＝記事本文が変わる変更）が先に必要。

**矛盾方向の偽陽性対策（公開21号・非feature記事101本での実測）:** 本文に「リメイク / リマスター」が現れる記事は101本中9本しかないが、そのうち **3件は他作品・別バージョンへの言及**で、語の有無だけでは区別できない。

- 「グレース…のパートは、初代『バイオハザード』の**リメイク**を彷彿とさせる」（vol.003）
- 「2024年には**リマスター**版『The Last of Us Part II Remastered』がリリースされ」（vol.005）
- 「PlayStation 4 向けに**リマスター**版が、PlayStation 5 向けにフル**リメイク**版…が発売されており」（vol.021）

そこで3つの絞り込みを掛けている。実測ではこの絞り込みで**上記3件すべてを抑止**し、発火したのは7箇所（4タイトル）でいずれも実際にリメイク / リマスターの作品だった。

1. **同一文スコープ + 「本作 / 同作」の主語**（`extractSentenceAt` は `platform-exclusivity-mismatch` と共用）。上記3件はいずれもこの語を持たない文なので落ちる
2. **ゲームタイトル内のマッチを除外**（`title` / `titleJa` 自体が「〜リマスター」を含む場合。vol.021 のトルネコ等）
3. **同一文に正しいラベルがあれば警告しない**。vol.015 の「…蘇る本作は、ただの**リマスター**ではなく、忠実に再構築された本格的な**リメイク**作品だ」がこれに該当し、否定表現（「〜ではなく」）をパースせずに抑止できる

**未検出のパターン:**
- 「本作 / 同作」を使わず、ゲームタイトルを主語にして種別を誤って述べる文（例: 「Test Game はリメイクである」）。上記の絞り込み1の代償。実測では本作/同作以外の主語で種別を述べる誤りは観測されていない
- 英語表記（`Remake` / `Remaster`）。`GAME_TYPE_LABELS` は執筆プロンプトに出す日本語ラベルの表なので、英語表記は走査対象に入らない。実測ではゲームタイトルの一部（`Final Fantasy VII Remake` 等）としての出現しかなく、種別の主張としての英語表記は0件
- `game_type` の 8 / 9 以外の値（Expanded Game / Port / Fork 等）。`GAME_TYPE_LABELS` に無い値は `expectedLabel` が undefined になり、未言及方向は発火しない（矛盾方向は「該当なし」として動く）
...
| 分類 | 件数 |
|------|------|
| メタデータの `genre` と一致 | 149 |
| **メタデータに無い（＝不一致として警告される）** | **37** |
| IGDB のジャンルに対応語が存在しない（`探索` 28 / `オープンワールド` 20 / `サバイバル` 18 / `ホラー` 14 等） | 132 |

不一致37件を全件文脈確認した内訳と、対応表の整備では解消しない理由:

- **部分一致で別の語を拾っただけ（6件）**: グ**レース**・アシュクロフト（人名）／レイト**レーシング**／スケート**ボードゲーム**／リ**アクション**演出／Beatspell **RPG**（ゲーム内モード名）。語境界処理で減らせるが、日本語に語境界が無いため完全には除けない
- **語の多義性（18件）**: **「プラットフォーム」14件はすべて誤検出**で、全件が「Steam などのプラットフォームで購入可能」「マルチプラットフォーム展開」の類（配信基盤・対応機種の意味であり、ジャンルの `Platform` ではない）。ほかに「発売元の Team17 はインディーゲームのパブリッシングで知られる」（他社の説明）、「アーケードゲームの古典に強く影響を受けた」（影響元）、「カードゲーム『グウェント』」（作中のミニゲーム）、「Svensson は以前シミュレーションゲームの開発に…」（開発者の経歴）
- **メタデータ側の欠落（13件）**: Portal 2 の `genres` に `Puzzle` が無い／Far Far West に `Shooter` が無い／「ターン制」の記述に対し `Turn-based strategy (TBS)` が無い。**記事は正しく、IGDB が足りていない**側なので執筆AIには直せない

「プラットフォーム」の多義性とメタデータ側の欠落は語彙の問題ではなく設計の問題であり、対応表を整備しても解消しない。検証を入れないので対応表自体もコードに入れていない（入れると dead code になる）。ジャンルの誤りは3章の LLM-judge 側で扱う。
...
なお種別の**正しさ**の検証自体は judge の外で担保している（2-2 の `game-type-unstated` / `game-type-mismatch`、Issue #387）。バリデータは `GeneratedArticle.game.gameType` を直接照合するため、judge に `gameType` を渡すかどうかとは独立に動く。ここで残るのは「種別の転記が judge の claim として無駄に消費される」というコスト側の話であって、検証の空白ではない。
