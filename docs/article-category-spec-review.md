...
> **Validation Report（`data/validation/validation-report-017 / 018 / 019`）は削除していない**ので、レポートを根拠にした引用は今も直接検証できる。ただし**レポートのファイルパス（`validation-report-0NN.*`）は号番号から生成されるため、同じ号番号を再生成すると上書きされる**（`validate-article.ts` がファイル名を issue number から決定）。vol.017 の report は 2026-08-14 のテスト実行（run 31792016284）で既に上書きされ、本 PR で `0a0709b` から復元された。**恒久的な参照方法は `git show 0a0709b:data/validation/validation-report-0NN.json`** である。
...
実装状況: **2026-08-08 に PR-B（PR #230）で 論点 N-6（3 軸スコア。国内販売軸は PR-B2）と 論点 J-1-c（リメイク明記）を実装した。**
          実装で判明した事実は 11.4.5 / 論点J-1 の各決着ブロック末尾に「実装時の実測」として追記してある。
...
>
> ✅ **実装（2026-08-09。PR #249）**: §2.3 の 4 条件のうち未実装だった 2 つ（批評媒体数 2 以上 / Amazon 掲載）を実装した。**素案の `aggregated_rating` 経路の追加だけでは #238 は解決しないという本節の指摘は正しかった。**『Splatoon Raiders』は `aggregated_rating_count = 1` で批評経路を満たさず、実際に Amazon 経路で救われた。Amazon 経路は `options` 引数で新作枠にのみ有効化し、インディー枠・名作枠・特集枠には渡していない。批評経路は全枠共通。批評経路が全枠に波及する点と評点しきい値が無い点は **#251** に分離（実測では批評経路だけで通るのは 146 件中 8 件で評点 74〜87）。
>
> ⚠️ **同 PR で `hasExistenceEvidence`（`select-newreleases-with-fallback.ts`）にも Amazon 経路を足した。** 本節の対象は `isQualifiedGame` だけだが、品質条件だけ通しても実存フィルタで落ちる構造（Steam 非掲載・IGDB 票数 5 未満の国内専用タイトル）が残るため。**これは「カバー画像がある／開発元が判明している」を要求する §2.4 の実在条件とは別レイヤーの短絡である**点に注意。
>
> ⚠️ **§2.3 に規定が無いのに残した経路は 3 つ**（本 PR では削除しない）: `metascore != null`（削除は PR-D 担当）/ `steamPlayers != null && steamPlayers > 0`（§2.3 が挙げるのは Top Sellers であって Top Played ではない。**担当 PR が未割り当て**）/ `igdbRating >= 85 && rc >= 8` の救済経路。

⚠️ **以下の段落は PR #249 より前の記述で、現在は成り立たない**（履歴として残す）。実際の分岐数は当時 **5 経路**（本文の「6 経路」は記載時点で既に誤り）で、PR #249 後は **7 経路**。§2.3 の 4 条件はすべて実装済みになった。
...
> ✅ **実装（2026-08-09。PR #237）**: `involved_companies.company.developed` の件数を静的リストと OR 併用（閾値 `> 20`、環境変数 `LARGE_STUDIO_DEVELOPED_THRESHOLD`）。**`companies` エンドポイントへの追加リクエストは不要**だった（`games` からの入れ子参照で取得できる）。実装時の実測で 2 つの限界が判明: **IGDB の会社レコード重複**（#236）と**多作な小規模スタジオの誤判定**（Kairosoft = 88 本。#240）。
...
>
> ✅ **実装（2026-08-09。PR #237）**: `select-newreleases-with-fallback.ts` の `isLargeStudio` ゲートを削除した。実測で新作枠の採用は **0 本 → 2 本**（『ほの暮しの庭』『MARVEL Tōkon: Fighting Souls』）。
>
> ⚠️ **決着から実装まで 11 日間、この決定はどの PR にも割り当てられていなかった。** その間に Issue #231 が「静的リストの不備」と診断し、**本論点で棄却された (A-3)「大手条件を維持して判定精度を上げる」に相当する対処**を提案していた。PR-I はその方針で着手されており、ユーザーの指摘（「新作紹介は大手かインディーかを問わない仕様ではなかったか」）で初めて誤りが判明した。**Issue の診断は決着ブロックと突き合わせて検証すること。**
...
>
> #### 実装結果（2026-08-08。PR #230）
>
> **実装済み。** 実装で判明したこと:
>
> - **未測定だった品質条件を通過することを実測で確認した。** 『アサシン クリード ブラック フラッグ RE:シンクロ』は `aggregated_rating_count = 4`（`aggregated_rating = 84` / `rating_count = 25`）で品質条件（媒体数 2 以上）を通る。(J-1-c) の効果は実在する
> - **「許可するだけ」では足りないのは、プロンプトだけの問題ではなかった。** `fetchIGDBData` が 3 つの母集団クエリを 1 プールに平坦化するため、新作枠クエリにだけ許可した種別が名作枠・インディー枠の選定にも流れ込む。実測で当該タイトルは名作枠の候補条件をすべて通過した（名作記事には種別を渡していないため、リメイクが原作の名作として発行される状態だった）。**選定側にもリメイク除外を置いて塞いだ**
> - **リメイクは Steam 経由では以前から記事化されていた。** クールダウン履歴に `gothic 1 remake` / `fatal frame ii crimson butterfly remake` / `アサシン クリード ブラック フラッグ re シンクロ` が実在する。IGDB の `game_type` フィルタは IGDB 由来候補しかゲートしていないため、本論点の「リメイクが載らない」は**経路によって成立していなかった**
> - **受け入れ条件（原作のストーリーに踏み込まないこと）は満たされた。** DEV_MODE の実出力で、記事は「2013 年の名作をリメイクした作品として」と明記しつつ、本文の記述（年号・技術・開発者の動機・レビュー引用）はすべて【外部参照データ】に根拠があった。検索結果に含まれていた開発者の個人名は記事に出ておらず、既存の禁止リストが機能している
...
| # | 項目 | 決定内容 | 根拠 |
|---|---|---|---|
| 1 | 新作紹介の企業規模条件 | **撤廃する**（論点A = A-1）。`select-newreleases-with-fallback.ts:56-68` の `isLargeStudio` AND ゲートを削除 → ✅ **2026-08-09 実装済み（PR #237）** | Palworld のような大型インディーが落とされる。規模はインディー枠の除外条件としてのみ使う（§5.1） |
...
| N-6（発売済み側のソート軸） | ✅ **決着**（2026-08-01。4 軸・絶対尺度・重み付き最大値）→ 11.4.5〜11.4.11。パラメータ再調整は Issue #210。**✅ 2026-08-08 に 3 軸で実装済み**（PR #230。国内販売軸は PR-B2） |
...
| インディー枠のソート軸 | ✅ **決着済み**（2026-08-07。**Steam おすすめ数の降順**。→ 仕様書 §3.6 / §9.2-1）。本行の「N-6 のスコア式を流用できるか要検討」「PR-B の実装時に決める」は決着前の記述で、**PR-B（#230）では扱っていない**（実装は PR-I の担当）。なお母集団クエリの `sort hypes desc` は 3 枠とも未変更のまま |
...
##### 11.4.5-b 実装時の実測（2026-08-08。PR #230）

**3 軸で実装した**（国内販売軸は PR-B2）。実装で判明したこと:

- **写像の数値に誤りがあった。** 票数軸のクリップ根拠として仕様書 §2.3 が挙げていた「5,000 票で 133」は誤りで、正しくは **137**（`100 × log10(5000) ÷ log10(500) = 137.05`）。1,000 票の 111 は正しい。結論（クリップは必須）は変わらない
- **`votesFull` は 1 以下にできない。** `log10(1) = 0` で全候補が `Infinity` → 100 点に張り付き、1 未満では符号が反転して軸の順序が逆になる。Issue #210 が運用中に回すノブなので、実装では `votesFull <= 1` を棄権として扱っている
- **棄権と 0 点は戻り値で区別できるようにした。** 保有軸の内訳（`axes`）を返し、空配列＝どの軸も持たない候補、という判別ができる。重み 0 で「保有しているが 0 点」との区別が付く
- **Steam 軸の分母**は `steamData.topSellers.length` を選定処理に渡している（11.4.9 の「ハードコードせず実際の取得件数から算出する」に対応）。実測時の取得件数は 20 件
- **どの軸も保有しない候補が多数を占める。** 実測（2026-08-08 の新作枠候補 23 件）では、Steam 由来の無名タイトルが `steamRank` だけを持ち、IGDB 指標を持つ候補はごく少数だった。最大値集約のもとでは Steam 軸が事実上の主軸になる号がありうる
